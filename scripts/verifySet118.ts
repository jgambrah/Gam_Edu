...
function verifySet118Local() {
  console.log('=== VERIFYING SET 118 (2025 BECE INTEGRATED SCIENCE VARIANT) ===');

  // 1. Paper 1 checks
  console.log('\nChecking Paper 1:');
  const p1Questions = SET_BECE_2025_SCIENCE_P1.questions;
  ...
  // 2. Paper 2 checks
  console.log('\nChecking Paper 2:');
  const p2Questions = SET_BECE_2025_SCIENCE_P2.questions;
  ...
  const svgs = [
    { name: 'Carbon Cycle (IMG_2602.jpg)', match: 'B (CO₂)' },
    { name: 'Farming Systems (IMG_2603.jpg)', match: 'System K' },
    { name: 'Inclined Plane (IMG_2604.jpg)', match: 'INCLINED PLANE' },
    { name: 'Separation Set-ups (IMG_2605.jpg)', match: 'Set-up A (Distillation)' }
  ];

  svgs.forEach(svg => {
    const found = q1.subQuestions.some(sub => sub.prompt.includes(svg.match));
    if (!found) {
  ...
  console.log('\n🎉 All Set 118 criteria successfully validated!');
}
