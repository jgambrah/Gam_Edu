...
// ============================================================================
// 4q. ALIGNED CORE CURRICULUM SERIES: JHS Math Objective Mastery Series (Set 43)
// Target: global_curriculum/jhs/subjects/math/topics/core_curriculum_mastery/question_sets/jhs-math-mastery-series-43
// ============================================================================
export const SET_JHS_MASTERY_SERIES_43: CurriculumQuestionSet = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'payloads', 'jhs-math-mastery-series-43.json'), 'utf-8')
);
...
// ============================================================================
// 4s. ALIGNED CORE CURRICULUM SERIES: JHS Math Objective Mastery Series (Set 45)
// Target: global_curriculum/jhs/subjects/math/topics/core_curriculum_mastery/question_sets/jhs-math-mastery-series-45
// ============================================================================
export const SET_JHS_MASTERY_SERIES_45: CurriculumQuestionSet = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'payloads', 'jhs-math-mastery-series-45.json'), 'utf-8')
);

// ============================================================================
// 4t. ALIGNED CORE CURRICULUM SERIES: JHS Math Structured Problem-Solving Series (Set 46)
// Target: global_curriculum/jhs/subjects/math/topics/core_curriculum_mastery/question_sets/jhs-math-mastery-series-46
// ============================================================================
export const SET_JHS_MASTERY_SERIES_46: CurriculumQuestionSet = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'payloads', 'jhs-math-mastery-series-46.json'), 'utf-8')
);
...
export async function runCurriculumSeeding() {
  ...
  // Seed Set 46: JHS Math -> Core Curriculum Series (Set 43 Objective in core_curriculum_mastery)
  console.log('▶ Ingesting Set 46: Junior Core Math Objective Mastery Series (Set 43)...');
  const result46 = await seedTopicSet(
    'jhs',
    'math',
    'core_curriculum_mastery',
    SET_JHS_MASTERY_SERIES_43
  );
  results.push(result46);

  // Synchronize Topic Manifest for Set 43
  if (db && !isDryRun) {
    const topicRef = db.doc('global_curriculum/jhs/subjects/math/topics/core_curriculum_mastery');
    await topicRef.set({
      questionSetIds: FieldValue.arrayUnion('jhs-math-mastery-series-43'),
      lastUpdated: FieldValue.serverTimestamp(),
    }, { merge: true });
  ...
  // Seed Set 48: JHS Math -> Core Curriculum Series (Set 45 Objective in core_curriculum_mastery)
  console.log('▶ Ingesting Set 48: Junior Core Math Objective Mastery Series (Set 45)...');
  const result48 = await seedTopicSet(
    'jhs',
    'math',
    'core_curriculum_mastery',
    SET_JHS_MASTERY_SERIES_45
  );
  results.push(result48);

  // Synchronize Topic Manifest for Set 45
  if (db && !isDryRun) {
    const topicRef = db.doc('global_curriculum/jhs/subjects/math/topics/core_curriculum_mastery');
    await topicRef.set({
      questionSetIds: FieldValue.arrayUnion('jhs-math-mastery-series-45'),
      lastUpdated: FieldValue.serverTimestamp(),
    }, { merge: true });
    console.log('✅ Topic manifest synchronized with jhs-math-mastery-series-45.');
  }

  console.log('\n----------------------------------------------------------------\n');

  // Seed Set 49: JHS Math -> Core Curriculum Series (Set 46 Structured in core_curriculum_mastery)
  console.log('▶ Ingesting Set 49: Junior Core Math Structured Problem-Solving Series (Set 46)...');
  const result49 = await seedTopicSet(
    'jhs',
    'math',
    'core_curriculum_mastery',
    SET_JHS_MASTERY_SERIES_46
  );
  results.push(result49);

  // Synchronize Topic Manifest for Set 46
  if (db && !isDryRun) {
    const topicRef = db.doc('global_curriculum/jhs/subjects/math/topics/core_curriculum_mastery');
    await topicRef.set({
      questionSetIds: FieldValue.arrayUnion('jhs-math-mastery-series-46'),
      lastUpdated: FieldValue.serverTimestamp(),
    }, { merge: true });
    console.log('✅ Topic manifest synchronized with jhs-math-mastery-series-46.');
  }
