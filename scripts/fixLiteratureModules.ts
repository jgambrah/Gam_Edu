...
function generatePayload(topicId: string, title: string) {
  return {
    id: topicId,
    topicId: topicId,
    subjectId: "english",
    ...
    levels: {
      b7: {
        levelTitle: "B7 • The Cockcrow Anthology & Literary Devices",
        summary: "Master the prescribed WAEC Cockcrow texts: Dickens' Oliver Twist, Aidoo's The Dilemma of a Ghost, short stories, prescribed poetry, and literary devices.",
      ...
      b8: {
        levelTitle: "B8 • The Cockcrow Anthology & Literary Devices",
      ...
      b9: {
        levelTitle: "B9 • The Cockcrow Anthology & Literary Devices",
      ...
      jhs1: {
        levelTitle: "JHS 1 • The Cockcrow Anthology & Literary Devices",
        summary: "Master the prescribed WAEC Cockcrow texts: Dickens' Oliver Twist, Aidoo's The Dilemma of a Ghost, short stories, prescribed poetry, and literary devices.",
        notes: B7_NOTES_MARKDOWN,
        workedExamples: B7_WORKED_EXAMPLES,
        practicePool: B7_PRACTICE_POOL
      ...
      jhs2: {
        levelTitle: "JHS 2 • The Cockcrow Anthology & Literary Devices",
        summary: "Victorian social critique, workhouse bureaucracy, and character allegories in 'Oliver Twist'.",
        notes: B8_NOTES_MARKDOWN,
        workedExamples: B8_WORKED_EXAMPLES,
        practicePool: B8_PRACTICE_POOL
      ...
      jhs3: {
        levelTitle: "JHS 3 • The Cockcrow Anthology & Literary Devices",
        summary: "Cross-cultural conflict in 'The Dilemma of a Ghost' and analysis of prescribed BECE poetry.",
        notes: B9_NOTES_MARKDOWN,
        workedExamples: B9_WORKED_EXAMPLES,
        practicePool: B9_PRACTICE_POOL
...
async function fixLiteratureModules() {
  const db = await getDb();
  console.log("Connecting to Firestore database...");

  const topicIds = [
    { id: "literature_cockcrow_canon", title: "Prose, Drama & Poetry Analysis" },
    { id: "cockcrow_literary_devices", title: "The Cockcrow Anthology & Literary Devices" }
  ];
  ...
  for (const topic of topicIds) {
    const payload = generatePayload(topic.id, topic.title);
    for (const parent of parentPaths) {
      const fullPath = `${parent}/${topic.id}`;
      await db.doc(fullPath).set(payload, { merge: true });
      console.log(`✅ Deployed full study notes & worked examples to: ${fullPath}`);
    }
  }

  // Update manifests/topical_labs
  ...
  if (mSnap.exists) {
    ...
    const updatedTopics = (mData.topics || []).map((t: any) => {
      if (t.id === 'literature_cockcrow_canon' || t.topicId === 'literature_cockcrow_canon' ||
          t.id === 'cockcrow_literary_devices' || t.topicId === 'cockcrow_literary_devices') {
        return {
          ...t,
          hasNotes: true,
          status: 'ready',
    ...
    await manifestRef.set({ ...mData, topics: updatedTopics, updatedAt: new Date().toISOString() }, { merge: true });
    console.log("✅ Manifest updated for literature topics.");
  }

  console.log("\n🎉 ALL DONE! Both literature_cockcrow_canon AND cockcrow_literary_devices now have full study notes and worked examples for JHS 1, JHS 2, and JHS 3!");
}

fixLiteratureModules()
  .then(() => process.exit(0))
  .catch((err) => {
