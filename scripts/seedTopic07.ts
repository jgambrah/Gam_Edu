...
async function seed() {
  const filePath = path.join(__dirname, 'payloads', 'topic_geometry_and_trigonometry.json');
  ...
  console.log(`\n================================================================`);
  console.log(`🚀 SEEDING TOPIC 07: "${payload.title}" [ID: ${payload.id}]`);
  console.log(`================================================================`);
  ...
  // Also sync alias document for topic_geometry_and_measurement to protect against alternate slug calls
  await db.doc('global_curriculum/jhs/subjects/math/topics/topic_geometry_and_measurement').set({
    ...payload,
    id: 'topic_geometry_and_measurement',
    updatedAt: FieldValue.serverTimestamp(),
  }, { merge: true });
  ...
  console.log(`🔄 Synced alias doc: topic_geometry_and_measurement`);
}
