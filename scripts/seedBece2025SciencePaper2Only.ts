...
import { SET_BECE_2025_SCIENCE_P2 } from '../src/lib/data/jhs-curriculum-set-119';
...
async function seedBece2025SciencePaper2Only() {
  console.log('Seeding 2025 BECE Integrated Science Paper 2 Variant (Set 119) into Firestore...');
  const db = await getFirestore();

  // 1. Update Paper 2 on main document: global_curriculum/jhs/subjects/science/past_papers/paper_2025_variant
  ...
  await mainDocRef.set({
    paper2: {
      ...
      totalQuestions: 5,
      questions: SET_BECE_2025_SCIENCE_P2.questions
    },
    metadata: {
      paper2Calibrated: true,
      set119Verified: true,
      sourceImages: ["IMG_2602.jpg", "IMG_2603.jpg", "IMG_2604.jpg", "IMG_2605.jpg"],
      updatedAt: adminInstance.firestore?.FieldValue ? adminInstance.firestore.FieldValue.serverTimestamp() : new Date()
    }
  }, { merge: true });

  console.log('✅ Successfully seeded Set 119 (Paper 2) into past_papers/paper_2025_variant.');

  // 2. Single-doc read paths for Paper 2 (Set 119)
  const p2Data = {
  ...
  console.log('🎉 Set 119 (2025 Science Paper 2 Variant) ingestion complete!');
}

seedBece2025SciencePaper2Only()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('Failed ingestion for Set 119 Science Paper 2:', err);
    process.exit(1);
  });
