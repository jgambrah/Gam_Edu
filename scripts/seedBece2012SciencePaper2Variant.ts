...
async function seedBece2012SciencePaper2Variant() {
  console.log('Seeding 2012 BECE Integrated Science Paper 2 Variant (Set 93) into Firestore...');
  ...
  await docRef.set({
    paper2: {
      ...
      totalQuestions: 5,
      questions: SET_BECE_2012_SCIENCE_P2.questions
    },
    metadata: {
  ...
  const topicDocRef = db.doc('global_curriculum/jhs/subjects/science/topics/bece_past_papers/question_sets/paper_2012_variant_p2');
  await topicDocRef.set({
    ...SET_BECE_2012_SCIENCE_P2,
    updatedAt: adminInstance.firestore?.FieldValue ? adminInstance.firestore.FieldValue.serverTimestamp() : new Date()
  }, { merge: true });
  console.log('✅ Ingested into topics/bece_past_papers/question_sets/paper_2012_variant_p2.');

  console.log('🌟 Successfully seeded Set 93 (2012 Science Paper 2 Variant) into Firestore.');
}

seedBece2012SciencePaper2Variant()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('Failed ingestion for Set 93 Science Paper 2:', err);
    process.exit(1);
  });
