...
const balancedQuestions: QuestionItem[] = [
  {
    "number": 1,
...
async function seedBece2019Paper1Variant() {
  console.log('Seeding 2019 BECE Paper 1 Variant (Set 66) into Firestore...');
  ...
  try {
    ...
    console.log('✅ Ingestion complete: 2019 Paper 1 Variant seeded with exact 10A/10B/10C/10D distribution into Firestore.');
  } catch (err: any) {
...
seedBece2019Paper1Variant()
  .then(() => process.exit(0))
  .catch(err => {
    console.error('Failed ingestion for Set 66 Paper 1:', err);
    process.exit(1);
  });
