...
async function auditMock5() {
  const db = await getDb();
  const docRef = db.doc('global_curriculum/jhs/subjects/science/mock_exams/mock_5');
  const snap = await docRef.get();
  if (!snap.exists) {
    console.error('FAIL: mock_5 does not exist in Firestore!');
    process.exit(1);
  }

  const data = snap.data();
  console.log('AUDIT REPORT: mock_5 in Firestore');
  ...
  console.log('\nTotal "predict" in entire Mock 5 document:', totalPredict);
  ...
  console.log('\n✅ ALL AUDIT CRITERIA PASSED: Mock 5 is perfectly calibrated, balanced, and intact.');
}
