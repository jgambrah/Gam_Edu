...
async function seedBece2022SciencePaper1Variant() {
  console.log('Seeding 2022 BECE Integrated Science Paper 1 Variant (Set 88) into Firestore...');

  const keyDist = { A: 0, B: 0, C: 0, D: 0 };
  balancedScience2022P1.forEach(q => {
  ...
  const docRef = db.doc('global_curriculum/jhs/subjects/science/past_papers/paper_2022_variant');
  await docRef.set({
    year: 2022,
    isVariant: true,
    setNumber: 88,
    ...
    paper1: {
      ...
      durationMinutes: 45,
      totalQuestions: 40,
      questions: balancedScience2022P1
    },
    metadata: {
      sanitized: true,
  ...
  console.log('✅ Ingestion complete: 2022 Science Paper 1 Variant seeded with exact 10A/10B/10C/10D distribution.');
}

seedBece2022SciencePaper1Variant()
  .then(() => process.exit(0))
  .catch(err => {
    console.error('Failed ingestion for Set 88 Science Paper 1:', err);
    process.exit(1);
  });
