...
async function verifySet95() {
  console.log('=== VERIFYING SET 95 (2011 BECE SCIENCE PAPER 2 VARIANT) ===');
  ...
  // Check Paper 1 preserved (Set 94 intact)
  if (data.paper1 && Array.isArray(data.paper1.questions)) {
    console.log('✅ Paper 1 preserved: ' + data.paper1.questions.length + ' questions (Set 94 intact)');
  ...
  if (q1cPrompt.includes('(Incisor)') || q1cPrompt.includes('(Molar)') || q1cPrompt.includes('(Canine)') || q1cPrompt.includes('I (Crown)') || q1cPrompt.includes('II (Root)')) {
    throw new Error('Spoiler labels found in Q1(c) SVG!');
  }
  if (q1dPrompt.includes('(Garden Fork)') || q1dPrompt.includes('(Spade)') || q1dPrompt.includes('(Hand Trowel)') || q1dPrompt.includes('(Watering Can)')) {
    throw new Error('Spoiler labels found in Q1(d) SVG!');
  }
  console.log('✅ All SVGs verified to be sanitized with 0 spoiler text.');

  // Verify Q1(a) rectilinear propagation worked solution
  const q1aWorked = q1.subQuestions[0].workedSolution;
  if (!q1aWorked.includes('straight lines') || !q1aWorked.includes('shadows') || !q1aWorked.includes('pinhole camera')) {
    throw new Error('Q1(a) optical phenomena explanation missing or incomplete!');
  }
  console.log('✅ Q1(a) rectilinear propagation, shadow phenomena, and pinhole camera application verified.');
  ...
  if (!q2.subQuestions || q2.subQuestions.length < 4) throw new Error('Q2 subquestions incomplete');
  ...
  const topicPath = 'global_curriculum/jhs/subjects/science/topics/bece_past_papers/question_sets/paper_2011_variant_p2';
  ...
  console.log('\n🌟 ALL VERIFICATION CHECKS PASSED FOR SET 95 (2011 BECE SCIENCE PAPER 2)!');
}
