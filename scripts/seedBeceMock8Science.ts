...
async function seedBeceMock8Science() {
  console.log('Seeding BECE Integrated Science Mock 8 (Set 139) into dedicated mock_exams/mock_8...');

  const db = await getDb();

  const keyDist = { A: 0, B: 0, C: 0, D: 0 };
  balancedMock8P1.forEach((q) => {
  ...
  console.log('Verified Mock 8 Paper 1 Key Distribution across 40 items:', keyDist);

  const docRef = db.doc('global_curriculum/jhs/subjects/science/mock_exams/mock_8');
  await docRef.set({
    mockId: "mock_8",
  ...
  console.log('✅ Ingestion complete: Set 139 (Mock 8) seeded successfully into mock_exams/mock_8.');
}

seedBeceMock8Science()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('Failed ingestion for Mock 8 Science:', err);
    process.exit(1);
  });
