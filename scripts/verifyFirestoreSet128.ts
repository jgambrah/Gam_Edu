...
async function runVerification() {
  console.log('🔍 Verifying Set 128 (1996 BECE Integrated Science Variant) in Firestore...');
  ...
  console.log('✅ Found paper_1996_variant document.');
  ...
  p2.questions.forEach((q: any, i: number) => {
  ...
  const q1c = p2.questions[0].subQuestions.find((sq: any) => sq.subId === '(c)');
  if (!q1c || !q1c.prompt.includes('<svg')) {
    throw new Error('❌ Q1(c) vector SVG missing from prompt!');
  }
  console.log('✅ Q1(c) Female reproductive system vector SVG verified.');

  const q2a = p2.questions[1].subQuestions.find((sq: any) => sq.subId === '(a)');
  if (!q2a || !q2a.prompt.includes('<svg')) {
    throw new Error('❌ Q2(a) vector SVG missing from prompt!');
  }
  console.log('✅ Q2(a) Solar eclipse ray tracing vector SVG verified.');

  const q3b = p2.questions[2].subQuestions.find((sq: any) => sq.subId === '(b)');
  if (!q3b || !q3b.prompt.includes('<svg')) {
    throw new Error('❌ Q3(b) vector SVG missing from prompt!');
  }
  console.log('✅ Q3(b) Oxygen Bohr atomic shell vector SVG verified.');
  ...
  console.log('🎉 ALL FIRESTORE VERIFICATION CHECKS PASSED FOR SET 128 (1996 BECE INTEGRATED SCIENCE)!');
}
