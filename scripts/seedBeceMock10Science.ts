...
async function seedBeceMock10Science() {
  console.log('Seeding BECE Integrated Science Mock 10 (Set 141) into dedicated mock_exams/mock_10...');

  const db = await getDb();

  const keyDist = { A: 0, B: 0, C: 0, D: 0 };
  balancedMock10P1.forEach((q) => {
  ...
  console.log('Verified Mock 10 Paper 1 Key Distribution across 40 items:', keyDist);

  const docRef = db.doc('global_curriculum/jhs/subjects/science/mock_exams/mock_10');
  await docRef.set({
    mockId: "mock_10",
  ...
  console.log('✅ Ingestion complete: Set 141 (Mock 10) seeded successfully into mock_exams/mock_10.');
}

seedBeceMock10Science()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('Failed ingestion for Mock 10 Science:', err);
    process.exit(1);
  });
