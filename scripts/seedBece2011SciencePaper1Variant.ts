...
async function seedBece2011SciencePaper1Variant() {
  console.log('Seeding 2011 BECE Integrated Science Paper 1 Variant (Set 94) into Firestore...');
  const db = await getFirestore();

  const keyDist = { A: 0, B: 0, C: 0, D: 0 };
  SET_BECE_2011_SCIENCE_P1.questions.forEach((q: any) => {
  ...
  // 1. Write to past_papers/paper_2011_variant
  const docRef = db.doc('global_curriculum/jhs/subjects/science/past_papers/paper_2011_variant');
  await docRef.set({
    year: 2011,
    isVariant: true,
    setNumber: 94,
    ...
    paper1: {
      ...
      totalQuestions: 40,
      format: "multiple_choice",
      questions: SET_BECE_2011_SCIENCE_P1.questions
    },
    metadata: {
      sanitized: true,
  ...
  console.log('✅ Ingested into past_papers/paper_2011_variant.');

  // 2. Write topic question set for single-document reads
  const topicRef = db.doc('global_curriculum/jhs/subjects/science/topics/bece_past_papers/question_sets/paper_2011_variant');
  await topicRef.set(SET_BECE_2011_SCIENCE_P1, { merge: true });
  console.log('✅ Ingested into topics/bece_past_papers/question_sets/paper_2011_variant.');

  console.log('🌟 Set 94 Ingestion completed with exact 10A/10B/10C/10D distribution.');
}

seedBece2011SciencePaper1Variant()
  .then(() => process.exit(0))
  .catch(err => {
    console.error('Failed ingestion for Set 94 Science Paper 1:', err);
    process.exit(1);
  });
