...
async function seedBece2019Paper2Variant() {
  console.log('Seeding 2019 BECE Paper 2 Variant (Set 67) into Firestore...');

  try {
    const docRef = db.doc('global_curriculum/jhs/subjects/math/past_papers/paper_2019_variant');
    ...
    await docRef.set({
      ...
      'metadata.set67Verified': true,
      'metadata.updatedAt': admin.firestore.FieldValue.serverTimestamp()
    }, { merge: true });

    console.log('✅ Successfully seeded Set 67 (2019 Paper 2 Variant) into past_papers/paper_2019_variant.');
  } catch (err: any) {
...
seedBece2019Paper2Variant()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('Failed ingestion for Set 67 Paper 2:', err);
    process.exit(1);
  });
