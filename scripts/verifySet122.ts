...
function verifySet122Local() {
  console.log('=== VERIFYING SET 122 (2013 BECE INTEGRATED SCIENCE VARIANT) ===');

  // 1. Paper 1 checks
  console.log('\nChecking Paper 1:');
  const p1Questions = SET_BECE_2013_SCIENCE_P1.questions;
  ...
  // 2. Paper 2 checks
  console.log('\nChecking Paper 2:');
  const p2Questions = SET_BECE_2013_SCIENCE_P2.questions;
  ...
  const svgs = [
    { name: 'Osmosis Living Tissue (IMG_2615.jpg)', match: 'LIVING PLANT TISSUE', forbidden: 'I (Yam cup)' },
    { name: 'Zinc-Acid Reaction (IMG_2616.jpg)', match: 'ACTION OF DILUTE ACID', forbidden: null },
    { name: 'Thermos Flask (IMG_2617.jpg)', match: 'THERMOS FLASK ANATOMY', forbidden: 'I (Cork/plastic stopper)' },
    { name: 'Soil Drainage Experiment (IMG_2618.jpg)', match: 'PHYSICAL PROPERTIES OF SOILS', forbidden: 'X (Sandy)' }
  ];
  ...
  svgs.forEach(svg => {
    ...
    if (svg.forbidden) {
      ...
      if (hasForbidden) {
        throw new Error(`SVG for ${svg.name} contains spoiler text: ${svg.forbidden}`);
      }
    }
    console.log(`✅ Reconstructed SVG verified & unlabelled: ${svg.name}`);
  });

  console.log('\n🎉 All Set 122 criteria successfully validated!');
}
