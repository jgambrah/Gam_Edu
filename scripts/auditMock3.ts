...
async function auditMock3() {
  const db = await getDb();
  const docRef = db.doc('global_curriculum/jhs/subjects/science/mock_exams/mock_3');
  const snap = await docRef.get();
  if (!snap.exists) {
    console.error('FAIL: mock_3 does not exist in Firestore!');
    process.exit(1);
  }

  const data = snap.data();
  console.log('AUDIT REPORT: mock_3 in Firestore');
  ...
  const q1Sub = data.paper2?.questions?.[0]?.subQuestions?.length;
  console.log('Paper 2 Section A Sub-questions Count:', q1Sub);
  ...
  console.log('✅ AUDIT PASSED: mock_3 is perfectly calibrated and standards-compliant!');
}

auditMock3().catch(err => {
