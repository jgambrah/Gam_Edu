...
async function auditMock4() {
  const db = await getDb();
  const docRef = db.doc('global_curriculum/jhs/subjects/science/mock_exams/mock_4');
  const snap = await docRef.get();
  if (!snap.exists) {
    console.error('FAIL: mock_4 does not exist in Firestore!');
    process.exit(1);
  }

  const data = snap.data();
  console.log('AUDIT REPORT: mock_4 in Firestore');
  ...
  console.log('✅ AUDIT PASSED: mock_4 is perfectly calibrated and standards-compliant!');
}

auditMock4().catch(err => {
