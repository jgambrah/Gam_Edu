...
async function auditMock7() {
  const db = await getDb();
  const docRef = db.doc('global_curriculum/jhs/subjects/science/mock_exams/mock_7');
  const snap = await docRef.get();
  if (!snap.exists) {
    console.error('FAIL: mock_7 does not exist in Firestore!');
    process.exit(1);
  }

  const data = snap.data();
  console.log('AUDIT REPORT: mock_7 in Firestore');
  ...
  console.log('\nTotal "predict" in entire Mock 7 document:', totalPredict);
  ...
  console.log('\n✅ ALL AUDIT CRITERIA PASSED: Mock 7 is perfectly calibrated, balanced, and intact.');
}
