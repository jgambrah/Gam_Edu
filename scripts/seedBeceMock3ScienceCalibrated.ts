...
const balancedMock3P1 = rawScienceBank.map((q, idx) => {
...
// ==========================================
async function seedBeceMock3ScienceCalibrated() {
  console.log('Connecting to Firestore and Seeding BECE Integrated Science Mock 3 (Set 134)...');
  const db = await getDb();

  // Verify balanced key distribution for Paper 1
  const keyDist = { A: 0, B: 0, C: 0, D: 0 };
  balancedMock3P1.forEach(q => {
  ...
  console.log('Verified Mock 3 Paper 1 Key Distribution across 40 items:', keyDist);
  ...
  await docRef.set({
    mockId: "mock_3",
    title: "BECE Integrated Science Mock 3 (Standard Mock Suite - NaCCA Calibrated)",
  ...
  console.log('✅ Ingestion complete: Set 134 (Mock 3) seeded successfully into mock_exams/mock_3.');
}

seedBeceMock3ScienceCalibrated()
  .then(() => process.exit(0))
  .catch(err => {
    console.error('Failed ingestion for Mock 3 Science:', err);
    process.exit(1);
  });
