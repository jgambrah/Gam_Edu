...
async function seedBeceMock11Science() {
  console.log('Seeding BECE Integrated Science Mock 11 (Electricity & Electronics Mastery - Set 142)...');

  const db = await getDb();

  const keyDist = { A: 0, B: 0, C: 0, D: 0 };
  balancedMock11P1.forEach((q) => {
  ...
  console.log('Verified Mock 11 Paper 1 Key Distribution across 40 items:', keyDist);

  const docRef = db.doc('global_curriculum/jhs/subjects/science/mock_exams/mock_11');
  await docRef.set({
    mockId: "mock_11",
  ...
  console.log('✅ Ingestion complete: Set 142 (Mock 11) seeded successfully into mock_exams/mock_11.');
}

seedBeceMock11Science()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('Failed ingestion for Mock 11 Science:', err);
    process.exit(1);
  });
