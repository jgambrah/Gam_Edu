...
import { SET_BECE_2004_SCIENCE_P2, svgQ1aCompleteCircuit, svgQ1bFractionalDistillation } from '../src/lib/data/jhs-curriculum-set-111';
...
async function seedBece2004SciencePaper2Only() {
  console.log('Seeding 2004 BECE Integrated Science Paper 2 Variant (Set 111) into Firestore...');
  ...
  await docRef.set({
    ...
    metadata: {
      ...
      set111Verified: true,
      sourcePhotographsIntegrated: ["IMG_2591.jpg", "IMG_2592.jpg", "IMG_2593.jpg", "IMG_2594.jpg"],
      updatedAt: adminInstance.firestore?.FieldValue ? adminInstance.firestore.FieldValue.serverTimestamp() : new Date()
    }
  }, { merge: true });

  console.log('✅ Successfully seeded Set 111 (2004 Science Paper 2 Variant) into past_papers/paper_2004_variant.');
  ...
  console.log('🎉 Set 111 (2004 Science Paper 2 Variant) dedicated ingestion complete!');
}

seedBece2004SciencePaper2Only()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error('Failed ingestion for Set 111 Science Paper 2:', err);
    process.exit(1);
  });
