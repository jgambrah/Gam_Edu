...
async function verifySet104() {
  console.log('=== VERIFYING SET 104 & SET 105 (2007 BECE INTEGRATED SCIENCE COMPLETE VARIANT) ===');
  ...
  if (data.setNumber !== 104) throw new Error('Expected setNumber 104, got: ' + data.setNumber);
  ...
  // Q06 Refraction bent stick SVG check
  const q06 = p1.questions[5];
  if (!q06.prompt.includes('<svg') || !q06.prompt.includes('Water Boundary')) {
    throw new Error('Q06 refraction bent stick SVG missing or incomplete');
  }
  console.log('Q06 refraction bent stick SVG verified.');
  ...
  // Q1a Single touch magnetization SVG
  if (!q1.subQuestions[0].prompt.includes('<svg') || !q1.subQuestions[0].prompt.includes('Steel Bar (AB)')) {
    throw new Error('Q1(a) single touch magnetization SVG missing');
  }
  console.log('Q1(a) single touch magnetization SVG verified.');

  // Q1b Over-water gas collection SVG
  if (!q1.subQuestions[1].prompt.includes('<svg') || !q1.subQuestions[1].prompt.includes('Thistle Funnel')) {
    throw new Error('Q1(b) over-water gas preparation SVG missing');
  }
  console.log('Q1(b) over-water gas preparation apparatus SVG verified.');

  // Q1c Exhaled breath in limewater SVG
  if (!q1.subQuestions[2].prompt.includes('<svg') || !q1.subQuestions[2].prompt.includes('Limewater Turns Milky')) {
    throw new Error('Q1(c) limewater exhalation SVG missing');
  }
  console.log('Q1(c) exhaled breath bubbling through limewater SVG verified.');
  ...
  // Q2c States of matter particles SVG
  const q2c = p2.questions[1].subQuestions[2];
  if (!q2c.prompt.includes('<svg') || !q2c.prompt.includes('Solid (Fixed)')) {
    throw new Error('Q2(c) states of matter sketches SVG missing');
  }
  console.log('Q2(c) states of matter sketches SVG verified.');

  // 4. Single-document read pattern
  const t1 = await db.doc('global_curriculum/jhs/subjects/science/topics/bece_past_papers/question_sets/paper_2007_variant').get();
  if (!t1.exists || t1.data().questions?.length !== 40) throw new Error('Topic doc paper_2007_variant missing or incomplete');
  console.log('Single-doc read paper_2007_variant verified (40 items).');

  const t2 = await db.doc('global_curriculum/jhs/subjects/science/topics/bece_past_papers/question_sets/paper_2007_variant_p2').get();
  if (!t2.exists || t2.data().questions?.length !== 5) throw new Error('Topic doc paper_2007_variant_p2 missing or incomplete');
  console.log('Single-doc read paper_2007_variant_p2 verified (5 questions).');

  console.log('\n🌟 ALL VERIFICATION CHECKS PASSED FOR SET 104 & SET 105 (2007 BECE SCIENCE COMPLETE VARIANT)!');
}

verifySet104()
  .then(() => process.exit(0))
  .catch(err => {
    console.error('Verification failed for Set 104:', err);
    process.exit(1);
  });
