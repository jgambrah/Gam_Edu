...
async function seedBece2017SciencePaper1Variant() {
  console.log('Seeding 2017 BECE Integrated Science Paper 1 Variant (Set 80) into Firestore...');
  const db = await getFirestore();

  const keyDist = { A: 0, B: 0, C: 0, D: 0 };
  balancedScience2017P1.forEach((q: any) => {
  ...
  await docRef.set({
    ...
    paper1: {
      ...
      durationMinutes: 45,
      totalQuestions: 40,
      questions: balancedScience2017P1
    },
    metadata: {
      sanitized: true,
  ...
  console.log('✅ Ingestion complete: 2017 Science Paper 1 Variant seeded with exact 10A/10B/10C/10D distribution.');
}

seedBece2017SciencePaper1Variant()
  .then(() => process.exit(0))
  .catch(err => {
    console.error('Failed ingestion for Set 80 Science Paper 1:', err);
    process.exit(1);
  });
