...
console.log('--- Verifying Set 131 (1999 BECE Integrated Science Variant) ---');

// 1. Verify Paper 1
console.log('\n[1] Verifying Paper 1:');
const p1Questions = SET_BECE_1999_SCIENCE_P1.questions;
...
// 2. Verify Paper 2
console.log('\n[2] Verifying Paper 2:');
const p2Questions = SET_BECE_1999_SCIENCE_P2.questions;
...
const q2a = p2Questions[1].subQuestions.find(sq => sq.subId === '(a)');
if (!q2a || !q2a.prompt.includes('<svg') || !q2a.prompt.includes('Limewater turns milky') || !q2a.prompt.includes('EXHALED CO₂ REACTS WITH LIMEWATER')) {
  throw new Error('Q2(a) missing or invalid Limewater breath test vector SVG!');
}
...
if (!q2d || !q2d.prompt.includes('<svg') || !q2d.prompt.includes('SECOND CLASS LEVER') || !q2d.prompt.includes('Wheelbarrow')) {
  throw new Error('Q2(d) missing or invalid Second Class Lever vector SVG!');
}
console.log('✅ Q2(d) Second class lever mechanics SVG verified.');

const q3b = p2Questions[2].subQuestions.find(sq => sq.subId === '(b)');
if (!q3b || !q3b.prompt.includes('<svg') || !q3b.prompt.includes('Fe + S') || !q3b.prompt.includes('Iron (II) Sulfide')) {
  throw new Error('Q3(b) missing or invalid Iron and Sulfur heating vector SVG!');
}
...
if (SET_BECE_1999_SCIENCE_COMPLETE.setNumber !== 131 || SET_BECE_1999_SCIENCE_COMPLETE.year !== 1999) {
  throw new Error('SET_BECE_1999_SCIENCE_COMPLETE metadata mismatch!');
}
console.log('✅ Complete model metadata verified.');

console.log('\n🎉 ALL LOCAL VERIFICATION CHECKS PASSED FOR SET 131 (1999 BECE INTEGRATED SCIENCE)!');
