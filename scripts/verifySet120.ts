...
function verifySet120Local() {
  console.log('=== VERIFYING SET 120 (2024 BECE INTEGRATED SCIENCE VARIANT) ===');

  // 1. Paper 1 checks
  console.log('\nChecking Paper 1:');
  const p1Questions = SET_BECE_2024_SCIENCE_P1.questions;
  ...
  // 2. Paper 2 checks
  console.log('\nChecking Paper 2:');
  const p2Questions = SET_BECE_2024_SCIENCE_P2.questions;
  console.log(`Total questions: ${p2Questions.length} (Expected: 5)`);
  if (p2Questions.length !== 5) {
    throw new Error(`Expected 5 questions in Paper 2, found ${p2Questions.length}`);
  }
  ...
  const svgs = [
    { name: 'Mammalian Heart (IMG_2606.jpg)', match: 'MAMMALIAN HEART' },
    { name: 'Planting Materials (IMG_2607.jpg)', match: 'Maize Grains' },
    { name: 'Measurement Circuit (IMG_2608.jpg)', match: 'II (Switch)' },
    { name: 'Separating Funnel (IMG_2609.jpg)', match: 'II (Stopcock)' },
    { name: 'Parallel Resistor Circuit (IMG_2612.jpg)', match: '4 Ω' }
  ];
  ...
  console.log('\n🎉 All Set 120 criteria successfully validated!');
}
