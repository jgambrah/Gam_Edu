...
async function verifyStrand3B8Foundation() {
  ...
  const subDocRef = db.doc(`global_curriculum/jhs/subjects/english/topical/${topicId}/practice_labs/B8_foundation`);
  ...
  if (mainDoc.exists) {
    ...
    const b8Pool = md.levels?.b8?.practicePool || {};
    console.log("Main doc b8 pool counts:", {
      low: b8Pool.low?.length || 0,
      medium: b8Pool.medium?.length || 0,
      hard: b8Pool.hard?.length || 0
    });
