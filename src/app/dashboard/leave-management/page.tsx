    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 animate-in fade-in-50 duration-200">
        ...
        <Button 
          ...
          className="w-full h-11 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-655 hover:to-purple-700 text-white font-extrabold text-xs rounded-xl shadow-md transition-all active:scale-[0.98] mt-2"
        >
            ...
            <div className="grid grid-cols-3 gap-4">
                ...
                <Card className="border border-amber-100 shadow-md bg-white rounded-2xl overflow-hidden hover:scale-[1.02] transition-all duration-300">
                    ...
                    <CardHeader className="pb-1 pt-5 px-5 flex flex-row items-center justify-between">
                      ...
                      <Clock className="h-4 w-4 text-amber-450" />
                    </CardHeader>
                    <CardContent className="pb-5 px-5">
                ...
                <CardContent className="p-0">
                     {isLoading ? (
                        <div className="flex justify-center p-24 flex-col items-center gap-3">
                            <Loader2 className="h-9 w-9 animate-spin text-indigo-505 opacity-30" />
                          ...
                          <Table>
                              ...
                              <TableBody>
                                  {myRequests?.map(req => {
                                      ...
                                      return (
                                          <TableRow key={req.id} className="hover:bg-slate-50/40 border-b border-slate-100 transition-colors h-16">
                                              ...
                                              <TableCell>
                                                  <div className="text-xs font-semibold text-slate-650">{format(startD, 'PPP')} - {format(endD, 'PPP')}</div>
                                                  <Badge variant="outline" className="mt-1 font-black text-[8px] border-indigo-200 text-indigo-650 bg-indigo-50/40 uppercase rounded-md py-0 px-1.5">{duration} Days duration</Badge>
                                              </TableCell>
                                              <TableCell className="max-w-xs truncate text-xs text-slate-505 font-semibold">{req.reason}</TableCell>
                                              <TableCell>
                                                  <Badge className={cn(
                                              ...
                                              <TableCell className="pr-8 text-right text-xs text-slate-505 font-semibold italic max-w-xs truncate">
                                  ...
                                  {(!myRequests || myRequests.length === 0) && (
                                      <TableRow>
                                          <TableCell colSpan={5} className="text-center py-20 text-slate-355 italic font-black uppercase tracking-[0.2em] text-xs">No leave requests found</TableCell>
                                      </TableRow>
                                  )}
                              </TableBody>
                          </Table>
         ...
         <Card className="rounded-3xl border border-slate-200 shadow-xl overflow-hidden bg-white">
            ...
            <CardContent className="p-6 grid md:grid-cols-2 gap-6 items-start">
                ...
                <div className="bg-slate-50 border border-slate-100 p-5 rounded-2xl h-full min-h-[300px] flex flex-col justify-between">
                    <div>
                      <h4 className="font-extrabold text-slate-805 text-xs uppercase tracking-wider mb-3 pb-1.5 border-b flex items-center justify-between">
                      ...
                      {selectedDateInfo.holiday && (
                        <div className="mb-4 bg-indigo-50 border border-indigo-100 p-3 rounded-xl flex items-center gap-2">
                           ...
                           <div>
                              ...
                              <p className="text-[10px] text-indigo-750 font-bold mt-0.5">{selectedDateInfo.holiday.name}</p>
                           </div>
                        </div>
                      )}
                         ...
                         <div className="space-y-2">
                            ...
                            <ul className="grid grid-cols-1 gap-2">
                                {selectedDateInfo.leaves.map((l, i) => (
                                  <li key={i} className="bg-white border border-slate-150 p-2.5 rounded-xl text-xs font-bold text-slate-805 flex items-center gap-2.5">
                    ...
                    <div className="mt-4 pt-3 border-t text-[9px] font-black font-mono text-slate-455 flex justify-between uppercase">
    ...
    return (
         <DialogContent className="rounded-3xl border-0 p-6 md:p-8 max-w-md shadow-2xl bg-white text-slate-800 animate-in fade-in-50 zoom-in-95">
            <DialogHeader className="pb-3 border-b">
                ...
                <DialogDescription className="font-semibold text-slate-505 text-xs">
                        ...
                        <FormItem>
                            ...
                            <FormControl>
                               <Textarea 
                                 ...
                                 className="rounded-xl border border-slate-200 min-h-20 text-xs font-semibold placeholder:text-slate-450 focus-visible:ring-indigo-500"
                                 {...field} 
                               />
                            </FormControl>
                            <FormMessage />
            ...
            <div className="flex gap-3 justify-end mt-6 pt-3 border-t">
                ...
                <Button 
                  ...
                  className={cn(
                    ...
                    action === 'Approve' ? "bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-655 hover:to-teal-700" : "bg-gradient-to-r from-rose-500 to-red-650 hover:from-rose-600 hover:to-red-750"
                  )}
                >
...
function ManagerLeaveView() {
  ...
  return (
    <div className="space-y-8 p-6 max-w-7xl mx-auto flex flex-col h-full">
        ...
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-slate-900 via-slate-955 to-indigo-955 text-white p-6 md:p-8 shadow-xl border border-slate-950/40">
          <div className="absolute right-0 top-0 opacity-5 pointer-events-none transform translate-x-12 -translate-y-12">
        ...
        <Card className="rounded-3xl border border-slate-200 shadow-xl overflow-hidden bg-white">
            <CardHeader className="bg-slate-50/50 border-b p-6 flex flex-row items-center justify-between">
              ...
              <Badge className="bg-amber-50 text-amber-705 border border-amber-200 uppercase font-black text-[9px] py-1 px-3">
                 ...
                 {isLoading ? (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in-50 duration-200">
                        {pendingRequests.map(req => {
                            ...
                            return (
                                <Card key={req.id} className="border border-slate-150 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col justify-between group hover:border-slate-355">
                                   <div className="p-5 space-y-4">
                                      <div className="flex items-center justify-between gap-3 pb-3 border-b">
                                         <div className="flex items-center gap-2.5">
                                            ...
                                            <div>
                                               <span className="font-extrabold text-slate-805 text-xs block group-hover:text-indigo-650 transition-colors">{req.staffName}</span>
                                      ...
                                      <div className="text-xs space-y-1 bg-slate-50 border p-3 rounded-xl font-semibold">
                                         ...
                                         <div className="flex justify-between items-center text-slate-505 border-t pt-1.5 mt-1">
                                            <span>Absence Duration:</span>
                                            <span className="text-indigo-650 font-extrabold">{duration} Days</span>
                                         </div>
                                      </div>

                                      <div className="text-xs text-slate-505 italic leading-relaxed border-l-2 pl-3 py-1 font-semibold border-slate-200">
                                         "{req.reason}"
                                      </div>
                                   </div>

                                   <div className="bg-slate-55 border-t p-3 flex gap-2.5">
                                      <Button 
                                         variant="outline" 
                                         size="sm" 
                                         onClick={() => openDialog(req, 'Reject')}
                                         className="flex-1 bg-white border-slate-200 hover:bg-rose-50 hover:text-rose-605 text-slate-650 hover:border-rose-200 font-extrabold text-xs rounded-xl h-10 transition-all active:scale-[0.98]"
                                      >
                                      ...
                                      <Button 
                                         ...
                                         className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-750 text-white font-extrabold text-xs rounded-xl h-10 shadow-sm transition-all active:scale-[0.98]"
                                      >
                      ...
                      <Table>
                          ...
                          <TableBody>
                                      ...
                                      <TableRow key={req.id} className="hover:bg-slate-50/40 border-b border-slate-100 transition-colors h-16">
                                          <TableCell className="pl-8">
                                              <div className="flex items-center gap-2.5">
                                                 ...
                                                 <div>
                                                    <span className="font-extrabold text-slate-805 block">{req.staffName}</span>
                                                    <span className="text-[9px] font-bold text-slate-400 uppercase font-mono">ID: {req.staffId.slice(0, 8)}</span>
                                                 </div>
                                              </div>
                                          </TableCell>
                                          <TableCell className="font-extrabold text-slate-805 text-xs">{req.leaveType}</TableCell>
                                          <TableCell>
                                              <div className="text-xs font-semibold text-slate-650">{format(startD, 'PPP')} - {format(endD, 'PPP')}</div>
                                          ...
                                          <TableCell className="pr-8 text-right text-xs text-slate-505 font-semibold italic max-w-xs truncate">
                                            {req.approverNotes ? `"${req.approverNotes}"` : <span className="text-slate-300">No notes recorded</span>}
                                          </TableCell>
                                      </TableRow>
                                  );
                              })}
                              {resolvedRequests.length === 0 && (
                                  <TableRow>
                                      <TableCell colSpan={5} className="text-center py-20 text-slate-355 italic font-black uppercase tracking-[0.2em] text-xs">No resolved requests found</TableCell>
                                  </TableRow>
                              )}
                          </TableBody>
                      </Table>
        ...
        <Card className="max-w-md w-full border-red-100 bg-red-50/50 rounded-3xl shadow-xl overflow-hidden">
            <CardHeader className="text-center p-8">
                <div className="bg-red-100 p-4 rounded-full w-fit mx-auto mb-4 animate-pulse">
                    <ShieldAlert className="h-8 w-8 text-red-655" />
                </div>
