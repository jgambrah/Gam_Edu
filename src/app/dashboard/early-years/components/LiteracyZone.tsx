...
const PhonicsZone: React.FC = () => {
  ...
  const playSound = async () => {
    ...
    setPlaying(true);
    const text = `The letter ${currentItem.upper} makes the sound ${currentItem.lower}... ${currentItem.lower}... ${currentItem.upper} is for ${currentItem.word}. Can you say ${currentItem.word}?`;
  ...
  return (
    <div className="relative">
      <button onClick={() => setIsAdminOpen(true)} className="absolute -top-12 right-0 bg-white border-2 border-pink-200 text-pink-400 px-4 py-2 rounded-full font-bold shadow-sm hover:bg-pink-50 transition-colors flex items-center gap-2">
        ...
        <div className="w-64 h-64 bg-gray-50 rounded-[2rem] overflow-hidden shadow-inner flex items-center justify-center relative mb-8 border-4 border-pink-50">
          {loading ? (
            ...
            <div className="text-gray-300 flex flex-col items-center">
               <i className="fas fa-image fa-4x mb-2"></i><p className="text-xs">No image found</p>
            </div>
          )}
        </div>
        <div className="flex gap-4 items-center">
          <button onClick={() => setCurrentIndex(prev => (prev === 0 ? phonicsList.length - 1 : prev - 1))} className="w-14 h-14 rounded-full bg-pink-100 text-pink-500 flex items-center justify-center hover:bg-pink-200 transition-colors shadow-sm"><i className="fas fa-chevron-left fa-lg"></i></button>
          <button onClick={playSound} disabled={playing} className={`w-28 h-28 rounded-full ${playing ? 'bg-pink-300' : 'bg-pink-500'} text-white flex items-center justify-center shadow-xl hover:scale-105 active:scale-95 transition-all border-4 border-white`}><i className={`fas ${playing ? 'fa-spinner fa-spin' : 'fa-volume-high'} fa-4x`}></i></button>
          <button onClick={() => setCurrentIndex(prev => (prev === phonicsList.length - 1 ? 0 : prev + 1))} className="w-14 h-14 rounded-full bg-pink-100 text-pink-500 flex items-center justify-center hover:bg-pink-200 transition-colors shadow-sm"><i className="fas fa-chevron-right fa-lg"></i></button>
        </div>
...
// VocabularyWorld: Teaches new words with images and sounds.
const VocabularyWorld: React.FC = () => {
    ...
    const playSound = async () => {
      ...
      setPlaying(true);
      const text = `${currentItem.word}. Can you say ${currentItem.word}?`;
      const result = await generateTTSAction({text, voice: 'Kore'});
      if (result.success && result.data) {
        const source = await playRawPcm(result.data);
        if (source) {
    ...
    return (
      <div className="relative">
        <button onClick={() => setIsAdminOpen(true)} className="absolute -top-12 right-0 bg-white border-2 border-green-200 text-green-500 px-4 py-2 rounded-full font-bold shadow-sm hover:bg-green-50 transition-colors flex items-center gap-2"><i className="fas fa-folder-plus"></i> Teacher's Drawer</button>
        <div className="max-w-xl mx-auto p-8 bg-white rounded-[3rem] shadow-2xl border-8 border-green-100 flex flex-col items-center">
          <div className="text-center mb-6"><span className="px-4 py-1 bg-green-50 text-green-600 rounded-full text-sm font-bold uppercase tracking-widest border border-green-100">{currentItem.category}</span><h2 className="text-6xl font-extrabold text-green-500 mt-4 mb-2 lowercase">{currentItem.word}</h2></div>
          <div className="w-72 h-72 bg-gray-50 rounded-[3rem] overflow-hidden shadow-inner flex items-center justify-center relative mb-8 border-4 border-green-50 group cursor-pointer" onClick={playSound}>
          ...
          <div className="flex gap-6 items-center"><button onClick={() => setCurrentIndex(prev => (prev === 0 ? vocabList.length - 1 : prev - 1))} className="w-16 h-16 rounded-full bg-green-50 text-green-500 flex items-center justify-center hover:bg-green-100 transition-colors shadow-md border-2 border-white"><i className="fas fa-arrow-left fa-xl"></i></button><button onClick={playSound} disabled={playing} className={`w-24 h-24 rounded-full ${playing ? 'bg-green-300' : 'bg-green-500'} text-white flex items-center justify-center shadow-xl hover:scale-105 active:scale-95 transition-all border-4 border-white`}><i className={`fas ${playing ? 'fa-spinner fa-spin' : 'fa-volume-high'} fa-3x`}></i></button><button onClick={() => setCurrentIndex(prev => (prev === vocabList.length - 1 ? 0 : prev + 1))} className="w-16 h-16 rounded-full bg-green-50 text-green-500 flex items-center justify-center hover:bg-green-100 transition-colors shadow-md border-2 border-white"><i className="fas fa-arrow-right fa-xl"></i></button></div>
          <div className="mt-8 flex items-center gap-2">{vocabList.map((_, idx) => (<div key={idx} className={`h-2 rounded-full transition-all ${idx === currentIndex ? 'w-8 bg-green-500' : 'w-2 bg-green-100'}`}></div>))}</div>
        </div>
...
// WritingCanvas: Allows kids to practice writing strokes, letters, and numbers.
const WritingCanvas: React.FC = () => {
    ...
    const handleFinish = async () => {
        ...
        setIsEvaluating(true); setFeedbackMessage('Magic checking...');
        try {
            ...
            if (result.success && result.data) {
                if (result.data.isCorrect) {
                  setShowSuccess(true); setFeedbackMessage('You are a star!');
                  await playSound(`Wow! You wrote the ${target} perfectly! You are a writing superstar!`);
                  setTimeout(() => setShowSuccess(false), 5000);
                } else {
                  setFeedbackMessage('So close! Try once more.');
                  await playSound(`Almost there! Let's try to trace the ${target} one more time. You can do it!`);
                }
            } else {
                throw new Error(result.error || 'AI check failed');
            }
        } catch (error) {
            console.error(error); setFeedbackMessage('The magic is sleeping.');
        } finally {
...
const SongBox: React.FC = () => {
    ...
    return (
      <div className="flex flex-col items-center p-6 bg-yellow-50 rounded-[3rem] shadow-2xl border-8 border-yellow-100 min-h-[600px]">
        ...
        <div className="mt-auto pt-8"><p className="text-xs text-yellow-800/40 text-center max-w-md">Note: Video generation requires a valid paid project API key from <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="underline ml-1">Google AI Studio</a>.</p></div>
      </div>
    );
};
