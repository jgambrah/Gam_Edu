 ...
 */
export function formatStudentId(student?: Partial<Student>): string {
 ...
 */
export function searchStudent(student: Partial<Student>, searchTerm: string): boolean {
 ...
 */
export function formatStudentBadge(student: Partial<Student>): string {
  return `${student.firstName} ${student.lastName?.charAt(0)}. - ${formatStudentId(student)}`;
}

// --- NEW FUNCTION TO BE ADDED ---
export async function generateNextStudentId(firestore: any): Promise<string> {
    const counterRef = doc(firestore, 'counters', 'students');
    const year = new Date().getFullYear();

    // In a transaction, get the current counter and increment it.
    const newIdNumber = await firestore.runTransaction(async (transaction: any) => {
        const counterDoc = await transaction.get(counterRef);
        if (!counterDoc.exists()) {
            // If the counter doesn't exist, start at 1
