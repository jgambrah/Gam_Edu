...
// --- Lazy Load PDF Components to ensure they only run on the client ---
...
// We need to lazy load these as well to avoid server-side execution
...
const ReportCardDocument = ({ student, assessments, year, term, rank, totalStudents, subjectsList }: any) => {
    ...
    const reportData = Object.values(assessments.reduce((acc: any, curr: Assessment) => {
        ...
        if (!acc[subId]) acc[subId] = { id: subId, name, total: 0, max: 0 };
    ...
    const average = reportData.length > 0 ? reportData.reduce((sum: number, i: any) => sum + i.pct, 0) / reportData.length : 0;
        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.infoContainer}>
                    <View style={styles.infoCol}><View style={styles.infoRow}><Text style={styles.label}>Name:</Text><Text style={styles.value}>{student.firstName} {student.lastName}</Text></View><View style={styles.infoRow}><Text style={styles.label}>Student ID:</Text><Text style={styles.value}>{student.id.slice(0, 8).toUpperCase()}</Text></View></View>
                    <View style={styles.infoCol}><View style={styles.infoRow}><Text style={styles.label}>Year:</Text><Text style={styles.value}>{year}</Text></View><View style={styles.infoRow}><Text style={styles.label}>Term:</Text><Text style={styles.value}>{term}</Text></View><View style={styles.infoRow}><Text style={styles.label}>Class:</Text><Text style={styles.value}>{student.classId || 'N/A'}</Text></View></View>
                </View>
...
export function GenerateReportCard({ student, assessments, year, term, rank, totalStudents, subjectsList }: {
    student: any; assessments: any; year: string; term: string; rank: number; totalStudents: number; subjectsList: any[];
}) {
    return (
        <PDFDownloadLink
            document={<ReportCardDocument student={student} assessments={assessments} year={year} term={term} rank={rank} totalStudents={totalStudents} subjects={subjectsList} />}
