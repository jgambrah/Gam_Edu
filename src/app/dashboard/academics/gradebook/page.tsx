...
// --- SUB-COMPONENT: Student Academics Detail (Updated for 50/50 + Averages + Rank) ---
function StudentGradesDetail({ 
    ...
    const globalSubjectStats = useMemo(() => {
        // Map<SubjectID, { studentScores: { [uid]: number } }>
        const stats: Record<string, { studentScores: Record<string, number> }> = {};
        ...
        assessments.forEach(a => {
             ...
             // Logic: We need to recreate the 50/50 math here to get accurate total
             // NOTE: This is a simplified sum. For strict 50/50 ranking, we'd need to group by type first.
             // For performance in this view, we'll sum raw points for ranking approximation, 
             // or ideally, use the calculated Total % if possible. 
             ...
             // Weighted contribution (Simplified: 100 points max per bucket)
             // We just sum raw scores for ranking across class.
             // Improvement: To be 100% accurate, we should replicate the full weighted logic per student per subject.
             // Let's do raw summation for speed:
    ...
    const subjectGrades = useMemo(() => {
        ...
        return Object.values(grouped).map((data) => {
            ...
            if (subStats) {
                ...
                // Approximate conversion to % for display
                // Note: This assumes everyone has same max score structure. 
                classAvg = avgRaw; // This is raw points average.
                
                // Rank
                const myScore = subStats.studentScores[student.uid];
                const sorted = scores.sort((a,b) => b - a);
                rank = sorted.indexOf(myScore) + 1;
            }
                        ...
                        {subjectGrades.map((sub) => {
                            ...
                            return (
                                <TableRow key={sub.id}>
                                    ...
                                    <TableCell className="text-center text-xs text-muted-foreground">{sub.rank} <span className="text-[10px]">/ {totalStudents}</span></TableCell>
  ...
  return (
    <div className="space-y-6 p-6">
      <Card className="border-t-4 border-t-indigo-600 shadow-sm">
        ...
        {showDebug && <div className="p-4 bg-yellow-50 border-b border-yellow-200 text-xs font-mono text-yellow-800 animate-in fade-in slide-in-from-top-2"><p><strong>DEBUG INFO:</strong></p><p>Selected Class: {selectedClassId || 'None'}</p><p>Assessments Found: {assessments ? assessments.length : 'Loading...'}</p></div>}
      </Card>
