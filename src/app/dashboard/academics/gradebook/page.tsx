...
function StudentGradesDetail({ 
    ...
    const subjectGrades = useMemo(() => {
        ...
        return Object.values(grouped).map((data) => {
            ...
            if (subStats) {
                const studentScores = Object.values(subStats.studentScores);
                const totalPossiblePoints = (data.caMax + data.examMax); // Simplified denominator
                
                if (studentScores.length > 0 && totalPossiblePoints > 0) {
                     const totalClassPoints = studentScores.reduce((sum, score) => sum + score, 0);
                     classAvg = (totalClassPoints / (studentScores.length * totalPossiblePoints)) * 100;
                }
                
                const myScore = subStats.studentScores[student.uid];
                const sorted = studentScores.sort((a,b) => b - a);
  ...
  return (
    <div className="space-y-6 p-6">
      <Card className="border-t-4 border-t-indigo-600 shadow-sm">
        ...
        {showDebug && <div className="p-4 bg-yellow-50 border-b border-yellow-200 text-xs font-mono text-yellow-800 animate-in fade-in slide-in-from-top-2"><p><strong>DEBUG INFO (Refresh Key: {refreshKey}):</strong></p><p>Selected Class: {selectedClassId || 'None'}</p><p>Selected Year/Term: {selectedYear} / {selectedTerm}</p><p>Assessments Found: {assessments ? assessments.length : 'Loading...'}</p><p>Subjects Loaded: {subjects ? subjects.length : 'Loading...'}</p><p>User: {user ? user.uid : 'No User'}</p></div>}
      </Card>
