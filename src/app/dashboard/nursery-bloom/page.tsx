                                ...
                                {group.sounds.map((sound, idx) => (
                                    ...
                                    >
                                        <button 
                                            ...
                                            className={`w-full aspect-square rounded-3xl border-b-8 font-black text-3xl shadow-sm hover:-translate-y-1 transition-all flex flex-col items-center justify-center ${group.color} bg-white`}
                                        >
                                            {sound}
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-in slide-in-from-bottom-4">
                        {rhymeFamilies.map(item => (
                            <div key={item.family} className="relative group bg-white p-6 rounded-3xl border-2 border-slate-100 shadow-sm text-center">
                                ...
                                {canEdit && item.isCustom && item.id && (
                                    <Button 
                                        ...
                                        className="absolute top-2 right-2 h-6 w-6 opacity-0 group-hover:opacity-100 text-red-300 hover:text-red-500 bg-slate-50 rounded-full"
                                    >
                                        <Trash2 className="w-3.5 h-3.5"/>
                                    </Button>
                                )}
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex gap-2 p-1 bg-green-50 rounded-2xl w-fit mx-auto border border-green-100">
                <Button variant={activeTab === 'explorer' ? 'default' : 'ghost'} onClick={() => { setActiveTab('explorer'); setWordIndex(0); }} className="rounded-xl font-bold">Explorer</Button>
                    ...
                    {canEdit && (
                        <Card className="p-4 border-2 border-green-200 bg-green-50/30 rounded-3xl space-y-3 shadow-inner">
                            <h4 className="font-black text-green-800 text-sm flex items-center gap-2">
                                <PlusCircle className="w-4 h-4" /> Add Word to Letter
                            </h4>
                            <form onSubmit={handleAddAbcWord} className="space-y-2">
                                <div className="grid grid-cols-2 gap-2">
                                    ...
                                    <div>
                                        ...
                                        <Input 
                                            ...
                                            className="bg-white rounded-lg h-8 text-xs px-2"
                                            required
                                        />
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        ...
                                        <Input 
                                            ...
                                            className="bg-white rounded-lg h-8 text-xs text-center px-2"
                                            required
                                        />
                                    </div>
                                    <div>
                                ...
                                <Button type="submit" disabled={isAddingAbc} className="w-full bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold text-xs h-8">
                                ...
                                <div className="p-8 text-center space-y-8 animate-in zoom-in relative group">
                                    {currentWordData.isCustom && canEdit && (
                                        <Button 
                                            size="icon" 
                                            variant="ghost" 
                                            className="absolute top-4 right-4 text-red-300 hover:text-red-500 hover:bg-red-50 rounded-full"
                                    ...
                                    <div className="bg-green-50 p-8 rounded-[40px] border-2 border-dashed border-green-200 animate-in fade-in duration-300" key={`${selectedLetter}-${wordIndex}`}>
                                        <div className="text-9xl mb-4">{currentWordData.emoji}</div>
                                        <h3 className="text-5xl font-black text-slate-800">{currentWordData.word}</h3>
                                        <p className="text-2xl font-bold text-green-500 mt-2 italic">Sound: "{currentWordData.phonic}"</p>
                                    </div>
                                    <div className="flex gap-4 justify-center">
                                        <Button onClick={() => {
      ...
      <Card className="w-full max-w-md bg-white border-4 border-orange-100 shadow-xl rounded-[40px] overflow-hidden">
        <CardContent className="p-8 flex flex-col items-center min-h-[300px] justify-center">
            ...
            {mode === 'shapes' && <div className="text-9xl text-blue-500 mb-6 drop-shadow-md">{question.a}</div>}
             ...
             {mode === 'time' && (
                <div className="w-32 h-32 rounded-full border-4 border-slate-800 flex items-center justify-center mb-6 relative bg-white shadow-inner">
                    {/* Hour Hand (shorter, rotating) */}
                    <div 
                        className="absolute top-1/2 left-1/2 w-1.5 h-8 bg-slate-800 rounded-full" 
                        style={{ 
                    ...
                    <div 
                        className="absolute top-1/2 left-1/2 w-1 h-12 bg-slate-600 rounded-full" 
                        style={{ 
                    ...
                    ></div>
                    {/* Center Pin */}
                    <div className="absolute top-1/2 left-1/2 w-3 h-3 bg-slate-800 rounded-full z-10 -translate-x-1/2 -translate-y-1/2"></div>
                    ...
                    <div className="absolute left-2 top-1/2 -translate-y-1/2 text-xs font-black text-slate-700">9</div>
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-black text-slate-700">3</div>
                </div>
            )}
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {savedStories?.map((s:any) => (
                        <Card key={s.id} className="cursor-pointer border-b-8 border-purple-200 hover:border-purple-400 hover:-translate-y-1 transition-all relative group rounded-3xl overflow-hidden">
                            ...
                            {canEdit && (
                                <Button size="icon" variant="ghost" className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-red-200 hover:text-red-500" onClick={(e) => { e.stopPropagation(); handleDelete(s.id); }}>
...
export default function JuniorCampusPage() {
  ...
  const pageModules = [
    { id: 'coach', name: 'Voice Coach', icon: <Mic className="w-6 h-6"/>, color: 'text-pink-600 bg-pink-100' },
    { id: 'phonics', name: 'Phonics Forest', icon: <Music className="w-6 h-6"/>, color: 'text-teal-600 bg-teal-100' },
    { id: 'abc', name: 'ABC Kingdom', icon: <Brain className="w-6 h-6"/>, color: 'text-green-600 bg-green-100' },
    { id: 'math', name: 'Math Playground', icon: <Calculator className="w-6 h-6"/>, color: 'text-orange-600 bg-orange-100' },
    { id: 'stories', name: 'Story Spark', icon: <BookOpen className="w-6 h-6"/>, color: 'text-purple-600 bg-purple-100' },
    { id: 'science', name: 'Science World', icon: <Atom className="w-6 h-6"/>, color: 'text-blue-600 bg-blue-100' },
    { id: 'art', name: 'Art Studio', icon: <Palette className="w-6 h-6"/>, color: 'text-cyan-600 bg-cyan-100' },
    { id: 'rewards', name: 'Sticker Book', icon: <Trophy className="w-6 h-6"/>, color: 'text-yellow-600 bg-yellow-100' },
  ];
      ...
      <div className="max-w-6xl mx-auto">
        <Tabs defaultValue="coach" className="w-full">
            <TabsList className="grid w-full grid-cols-8 h-24 bg-white p-2 rounded-2xl shadow-sm border-2 border-slate-100 mb-8 overflow-x-auto">
                {pageModules.map(mod => (
                    <TabsTrigger 
                        key={mod.id}
                        value={mod.id} 
                        className={`rounded-xl data-[state=active]:${mod.color} font-bold flex flex-col items-center gap-1 text-xs md:text-sm`}
                    >
                        {mod.icon} {mod.name}
                    </TabsTrigger>
                ))}
            ...
            <div className="min-h-[500px]">
                ...
                <TabsContent value="phonics" className="mt-0"><div className="bg-white p-8 rounded-3xl shadow-xl border-b-8 border-teal-200"><PhonicsForest canEdit={canEdit} /></div></TabsContent>
                <TabsContent value="abc" className="mt-0"><div className="bg-gradient-to-b from-green-50 to-white p-8 rounded-3xl shadow-xl border-b-8 border-green-200"><ABCKingdom canEdit={canEdit} /></div></TabsContent>
