...
// --- 2.9 ABC MATCHER GAME COMPONENT ---
type MatcherMode = 'upper-to-lower' | 'letter-to-object' | 'sound-to-letter' | 'confusing-pairs';

interface MatcherChoice {
    id: string;
    label: string;
    emoji?: string;
    isCorrect: boolean;
...
function getSmartDistractorLetters(targetLetter: string, allAlphabet: string[]): string[] {
    ...
    // Special pedagogical prioritization for 't': always include stroke-confusions 'f', 'l', and 'd' or 'b'
    if (lowerTarget === 't') {
        const secondary = Math.random() < 0.5 ? 'd' : 'b';
        return shuffleArray(['f', 'l', secondary]);
    }
...
function AbcMatcherGame({
    ...
    const [matcherMode, setMatcherMode] = useState<MatcherMode>('upper-to-lower');
    const [currentLetter, setCurrentLetter] = useState<string>(selectedLetter || 'A');
    const [choices, setChoices] = useState<MatcherChoice[]>([]);
    ...
    const generateQuestion = useCallback((mode: MatcherMode, target: string) => {
        ...
        let correctChoice: MatcherChoice;
        ...
        if (mode === 'confusing-pairs') {
        ...
        } else if (mode === 'letter-to-object') {
            const upperTarget = target.toUpperCase();
            const targetWords = mergedDict[upperTarget] || [{ word: 'Apple', emoji: '🍎', phonic: 'ah' }];
            const correctEntry = targetWords[0] || { word: 'Apple', emoji: '🍎', phonic: 'ah' };

            correctChoice = {
                id: `correct_${correctEntry.word}`,
                label: correctEntry.word,
                emoji: correctEntry.emoji,
                isCorrect: true,
            };
            ...
            distractorChoices = otherLetters.map((l, i) => {
                ...
                return {
                    id: `distractor_${item.word}_${i}`,
                    label: item.word,
                    emoji: item.emoji,
                    isCorrect: false,
                };
            });
        } else if (mode === 'sound-to-letter') {
            const upperTarget = target.toUpperCase();
            correctChoice = {
                id: `correct_${upperTarget}`,
                label: upperTarget,
                isCorrect: true,
            };
            ...
            // 'upper-to-lower'
            const upperTarget = target.toUpperCase();
            const lowerTarget = upperTarget.toLowerCase();

            correctChoice = {
                id: `correct_${lowerTarget}`,
                label: lowerTarget,
                isCorrect: true,
            };
    ...
    const handleChoiceClick = async (choice: MatcherChoice, index: number) => {
        if (isRoundLocked || disabledIndices.includes(index)) return;
            ...
            setTimeout(async () => {
                if (nextStreak >= 5) {
                    ...
                    if (user && firestore) {
                        try {
                            await addDoc(collection(firestore, 'junior_stickers'), {
                                userId: user.uid,
                                emoji: '🌟',
                                name: 'ABC Matcher Champion',
                                category: 'literacy',
                                description: 'Mastered 5 rounds in ABC Kingdom Matcher Game!',
                                earnedAt: serverTimestamp(),
                                createdAt: serverTimestamp()
                            });
                            toast({
                                title: "🌟 New Sticker Awarded!",
                                description: "ABC Matcher Champion has been added to your Sticker Book!"
                            });
                        } catch (e) {
                    ...
                    if (matcherMode === 'confusing-pairs') {
                        ...
                        onLetterChange(nextTricky.toUpperCase());
                    } else {
                        const nextAlphabet = alphabet[Math.floor(Math.random() * alphabet.length)];
                        setCurrentLetter(nextAlphabet);
    ...
    // Reset game to play another 5-round cycle
        ...
        <div className="py-2 px-2.5 sm:py-3 sm:px-4 space-y-2 sm:space-y-2.5 max-w-md sm:max-w-lg mx-auto w-full select-none">
            ...
            <div className="w-full flex flex-nowrap items-center justify-between sm:justify-center gap-1 p-1 bg-emerald-50/80 rounded-xl border border-emerald-200 shadow-inner overflow-x-auto scrollbar-none">
                {[
                    ...
                    { id: 'sound-to-letter', icon: '🔊', label: 'Sound Match' },
                    { id: 'confusing-pairs', icon: '🎯', label: 'Tricky Pairs' },
                ].map(modeTab => (
                    <button
                        key={modeTab.id}
            ...
            <div className="flex items-center justify-between bg-amber-50/90 border border-amber-200 px-3 py-1 rounded-xl shadow-inner">
                ...
                <div className="space-y-1.5 sm:space-y-2 text-center">
                    ...
                    <div className="flex items-center justify-center gap-2 sm:gap-2.5 px-1">
                        ...
                        <Button
                            ...
                            className={cn(
                                "h-9 w-9 sm:h-10 sm:w-10 rounded-full p-0 shrink-0 shadow-md border-2 border-amber-400 bg-gradient-to-br from-amber-300 via-amber-400 to-yellow-400 hover:from-amber-400 hover:to-yellow-500 text-amber-950 flex items-center justify-center transition-all active:scale-90",
                                isSpeakingPrompt ? "scale-110 ring-4 ring-amber-300 ring-offset-1 animate-pulse" : "hover:scale-105"
                            )}
                        >
                            <Volume2 className={cn("w-4 h-4 sm:w-5 sm:h-5 text-amber-950", isSpeakingPrompt && "animate-bounce")} />
                        </Button>
                    </div>
                        ...
                        <div className="relative bg-gradient-to-b from-amber-50 via-yellow-50/60 to-emerald-50/40 border-2 border-amber-300/90 rounded-2xl p-1.5 sm:p-2 shadow-md max-w-[170px] sm:max-w-[190px] mx-auto flex flex-col items-center justify-center gap-0.5">
                            ...
                            {matcherMode === 'sound-to-letter' ? (
                                <div className="flex flex-col items-center gap-0.5 py-0.5">
                                    ...
                                    <span className="text-lg sm:text-xl font-black text-emerald-700 tracking-wide">
                                        /{currentPhonic}/
                                    </span>
                                </div>
                            ) : (
                            ...
                            {/* Dynamic Sub-label without spoilers */}
                            <div className="text-[10px] font-bold text-slate-500 flex items-center justify-center gap-1 py-0.5">
                                {matcherMode === 'upper-to-lower' && (
                                    <span>Capital Letter {currentPhonic ? `(Sound: /${currentPhonic}/)` : ''}</span>
                                )}
                                {matcherMode === 'letter-to-object' && (
                                    <span>What starts with this?</span>
                                )}
                                {matcherMode === 'sound-to-letter' && (
                                    <span>Match the Sound</span>
                                )}
                                {matcherMode === 'confusing-pairs' && (
                                    <span>Look closely at the shape!</span>
                                )}
                            </div>
                        </div>
                    </div>
