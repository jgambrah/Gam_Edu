...
import { 
  ...
  Code2, Bot, Trash2, BookOpen, CornerDownLeft, ArrowRight, Loader2, Eraser, AlertCircle 
} from 'lucide-react';
...
// import useSound from 'use-sound'; // UNCOMMENT IF YOU INSTALL THE LIBRARY
...
export default function LogicLabPage() {
  ...
  // Sounds (Optional: Add your own .mp3 files in public folder)
  ...
  const getBlockColor = (text: string) => {
    ...
    if (text.includes('=') && !text.includes('==')) return 'bg-orange-600 border-orange-700'; // Variables
    return 'bg-indigo-600 border-indigo-700'; // Default
  };
  ...
  const handleRun = async () => {
    ...
    // Clear previous state before running
    setConsoleOutput(''); 
    setLastResult(null);
    setIsRunning(true);
    ...
    if (result.success) {
        ...
        // Normalize for comparison (ignores spaces and case)
        ...
        // Success Logic
        const isSuccess = normOutput === normExpected || (activeMission.expectedOutput && result.output.includes(activeMission.expectedOutput));
                         ...
                         {missions.map(m => {
                             ...
                             return (
                                 <button
                                    key={m.id}
                                    disabled={isLocked}
                                    onClick={() => setCurrentMissionIndex(m.id)} // Effect hook handles reset
