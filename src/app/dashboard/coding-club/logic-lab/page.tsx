...
import { 
  ...
  Code2, Bot, Trash2, BookOpen, CornerDownLeft, ArrowRight, Loader2, Eraser, AlertCircle, FlaskConical, Trophy, Info 
} from 'lucide-react';
...
export default function LogicLabPage() {
  ...
  const [isReferenceOpen, setIsReferenceOpen] = useState(false); // NEW
  ...
  const handleAskCoach = async () => {
      ...
      setUserQuestion('');
      const response = await getCodeCoachResponseAction({ currentBlocks: workspaceBlocks, availableBlocks: dynamicBlocks, missionTitle: activeMission.title, userQuestion: userQuestion });
      ...
      <div className="flex items-center justify-between bg-white p-3 rounded-lg border shadow-sm">
        ...
        <div className="flex-1 mx-8">
             ...
             <div className="w-full bg-slate-100 rounded-full h-2.5">
                 <div className="bg-green-600 h-2.5 rounded-full transition-all duration-1000" style={{ width: `${progressPercentage}%` }}></div>
             </div>
        </div>
      ...
      <div className="flex flex-1 gap-4 overflow-hidden">
        ...
        <Card className="w-1/4 flex flex-col h-full bg-slate-50 border-r-0 rounded-r-none">
            <div className="p-4 border-b bg-white">
                <h2 className="font-bold text-xl flex items-center gap-2 text-indigo-700"><Code2 className="h-6 w-6"/> Logic Lab</h2>
            </div>
                                ...
                                return (
                                    ...
                                    >
                                        <span className="flex items-center gap-2">
                                            {isLocked ? <Lock className="h-3 w-3"/> : <span>{allMissions.findIndex(am => am.id === m.id) + 1}.</span>}
                                            {m.title}
                                        </span>
            ...
            <Card className="shrink-0 bg-white border-l-4 border-l-indigo-500">
                <CardHeader className="py-4">
                    <div className="flex justify-between items-start">
                        ...
                        <Button variant="outline" size="sm" onClick={explainTheory}><BookOpen className="h-4 w-4 mr-2"/> Explain Concept</Button>
                    </div>
            ...
            <div className="flex-1 flex gap-4 min-h-0">
                <div className="w-1/3 bg-slate-100 rounded-lg p-4 flex flex-col gap-2 overflow-y-auto border">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                        <Button variant="outline" className="border-slate-300 bg-white hover:bg-slate-100" onClick={() => handleAddBlock('[NEWLINE]')}><CornerDownLeft className="h-3 w-3 mr-1"/> Enter</Button>
                        <Button variant="outline" className="border-slate-300 bg-white hover:bg-slate-100" onClick={() => handleAddBlock('    ')}><ArrowRight className="h-3 w-3 mr-1"/> Indent</Button>
                    </div>
                ...
                <div className="flex-1 flex flex-col gap-4">
                    <div className="flex-1 bg-white rounded-lg border-2 border-dashed border-slate-300 p-4 overflow-y-auto relative font-mono text-sm">
                        {workspaceBlocks.length === 0 && (
                            <div className="absolute inset-0 flex items-center justify-center text-slate-400 pointer-events-none">Click blocks to build your code here</div>
                        )}
                        <div className="flex flex-wrap items-center gap-2 content-start">
                            {workspaceBlocks.map((block, i) => {
                                if (block === '[NEWLINE]') return <div key={i} className="w-full h-6 border-b border-slate-200 mb-2 relative group flex items-end"><span className="absolute right-0 top-1 text-xs text-slate-300 select-none">↵</span><button onClick={() => handleRemoveBlock(i)} className="absolute left-0 top-1 text-red-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 className="h-3 w-3"/></button></div>;
                                if (block === '    ') return <div key={i} className="w-8 h-8 bg-slate-50 border border-slate-200 rounded flex items-center justify-center text-slate-300 group relative"><span className="text-xs">→</span><button onClick={() => handleRemoveBlock(i)} className="absolute -top-1 -right-1 bg-white rounded-full text-red-500 opacity-0 group-hover:opacity-100 border shadow-sm z-10"><Trash2 className="h-3 w-3"/></button></div>;
                                return (
                                    <div key={i} className={`group relative text-white px-3 py-2 rounded shadow-md cursor-grab active:cursor-grabbing hover:opacity-90 transition-all ${getBlockColor(block)}`}>
                                        {block}
                                        <button onClick={() => handleRemoveBlock(i)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm z-10"><Trash2 className="h-3 w-3"/></button>
                                    </div>
                                );
                            })}
                        </div>
                    ...
                    <div className="flex gap-2">
                        <Button onClick={() => handleRun('test')} disabled={isRunning} className="bg-blue-600 hover:bg-blue-700 shadow-sm"><FlaskConical className="mr-2 h-4 w-4"/> Test Run</Button>
                        <Button onClick={() => handleRun('submit')} disabled={isRunning} className="flex-1 bg-green-600 hover:bg-green-700 shadow-sm">{isRunning ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Play className="mr-2 h-4 w-4"/>} Submit Mission</Button>
                        <Button variant="outline" onClick={handleHardReset} className="text-red-600 hover:bg-red-50"><Eraser className="h-4 w-4"/></Button>
                        <Button variant="outline" onClick={() => setIsCoachOpen(true)} className="text-purple-600 border-purple-200 bg-purple-50"><Bot className="mr-2 h-4 w-4"/></Button>
                    </div>

                    <div className="flex flex-col gap-2">
                        <div className="h-32 bg-black rounded-lg p-3 font-mono text-sm text-green-400 overflow-y-auto shadow-inner relative">
                            <div className="flex justify-between items-center opacity-50 border-b border-green-900 mb-2 pb-1 text-xs"><span>TERMINAL OUTPUT</span><button onClick={() => setConsoleOutput('')} className="hover:text-white cursor-pointer">Clear</button></div>
                            <pre className="whitespace-pre-wrap">{consoleOutput}</pre>
                        </div>
                        {lastResult && !lastResult.success && (
                            <div className="bg-red-50 border border-red-200 rounded-lg p-3 animate-in slide-in-from-bottom-2 fade-in">
                                <div className="flex items-center gap-2 text-red-700 font-bold text-sm mb-2"><AlertCircle className="h-4 w-4"/> Mission Failed</div>
                                <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                                    <div><span className="text-slate-500 block mb-1">Expected:</span><div className="bg-green-100 text-green-800 p-2 rounded border border-green-200">{lastResult.expected}</div></div>
                                    <div><span className="text-slate-500 block mb-1">Your Output:</span><div className="bg-red-100 text-red-800 p-2 rounded border border-red-200 break-words">{lastResult.actual || "(Nothing)"}</div></div>
                                </div>
                            </div>
                        )}
                    </div>
      ...
      <Dialog open={isCoachOpen} onOpenChange={setIsCoachOpen}>
          <DialogContent className="sm:max-w-[400px]">
              ...
              <div className="h-[300px] bg-slate-50 rounded border p-3 overflow-y-auto space-y-3">
                  {coachChat.map((msg, i) => (<div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] p-2 rounded text-xs ${msg.role === 'user' ? 'bg-blue-500 text-white' : 'bg-white border text-slate-700'}`}>{msg.text}</div></div>))}
                  {isCoachThinking && <div className="text-xs text-slate-400 animate-pulse">Coach is typing...</div>}
              </div>
              <DialogFooter>
                  <div className="flex w-full gap-2"><Input value={userQuestion} onChange={e => setUserQuestion(e.target.value)} placeholder="Ask for a hint..." className="text-xs"/><Button onClick={handleAskCoach} size="sm" disabled={isCoachThinking}><HelpCircle className="h-4 w-4"/></Button></div>
              </DialogFooter>
          </DialogContent>
      </Dialog>

      {/* NEW: REFERENCE SHEET MODAL */}
