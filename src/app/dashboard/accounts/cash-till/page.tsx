                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
                        <FormField control={form.control} name="amount" render={({ field }) => (
                            <FormItem><FormLabel>Adjustment Amount (GH₵)</FormLabel><FormControl><Input type="number" step="0.01" {...field} placeholder="e.g., -630 for over-recording" /></FormControl><FormMessage /></FormItem>
                        )} />
...
```
<changes>
  <description>Replaced the old cash till logic with the new, more robust implementation that includes till adjustments and bank transaction approvals. All Firestore queries and document writes have been updated to be SaaS-compliant by including the schoolId, ensuring data isolation for each school.</description>
  <change>
    <file>src/app/dashboard/accounts/cash-till/page.tsx</file>
    <content><![CDATA[
'use client';
...
// --- Accountant's Till View ---
function AccountantTillView({ setSelectedTill }: { setSelectedTill: (till: Till) => void }) {
    const { user } = useUser();
    const firestore = useFirestore();
    const { toast } = useToast();
    const [isSubmitting, setIsSubmitting] = useState(false);
        ...
        <Tabs defaultValue="active">
            ...
            <TabsContent value="active" className="mt-4">
                 {!activeTill ? (
                    ...
                    <TillAdjustmentDialog activeTill={activeTill} open={isAdjustmentOpen} setOpen={setIsAdjustmentOpen} onUpdate={refetchTransactions} />
                    </>
                )}
            </TabsContent>
            <TabsContent value="history" className="mt-4">
...
// --- Director's View: Till Detail Dialog ---
function TillDetailDialog({ till, open, setOpen, onUpdate, students, classes }: { 
    till: Till, 
    open: boolean, 
...
export default function CashTillPage() {
    ...
    return (
        <div className="space-y-6">
            <h1 className="text-3xl font-bold">Financial Submissions</h1>
            {isDirector && <DirectorTillView setSelectedTill={setSelectedTill} />}
            {isAccountant && <AccountantTillView students={null} classes={null} setSelectedTill={setSelectedTill} />}
            
            {/* The TillDetailDialog for Director will need student/class data if they are to see names */}
            <TillDetailDialog
                till={selectedTill}
                open={!!selectedTill}
