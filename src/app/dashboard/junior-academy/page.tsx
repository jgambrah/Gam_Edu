  ...
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
    <div className="bg-white rounded-[3rem] p-10 max-w-md w-full shadow-2xl border-8 border-gray-50 animate-in zoom-in duration-300 font-black">
      ...
      <div className="space-y-6">
        ...
        <button onClick={onClose} className="w-full py-2 text-slate-400 uppercase text-[10px] font-black hover:text-slate-600 block text-center transition-colors">Close Drawer</button>
      </div>
    </div>
  </div>
);
    ...
    return (
      <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-pink-100 flex flex-col items-center animate-in zoom-in font-black">
        ...
        <div className="flex gap-4 mt-12">
          <button onClick={() => setIndex(i => (i === 0 ? songs.length - 1 : i - 1))} className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center"><ArrowLeft /></button>
          <button onClick={() => setIndex(i => (i + 1) % songs.length)} className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center"><ArrowRight /></button>
        </div>
      </div>
    );
};
...
const ModelingModule: React.FC<{ onSound: (t: string) => void; onComplete: () => void }> = ({ onSound, onComplete }) => {
    ...
    useEffect(() => { fetchVisual(); }, [fetchVisual]);
...
const PracticalLifeModule: React.FC<{ onSound: (t: string) => void; onComplete: () => void }> = ({ onSound, onComplete }) => {
    ...
    useEffect(() => { fetchVisual(); }, [subTab, index, fetchVisual]);
    ...
    return (
      <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-blue-100 flex flex-col items-center animate-in zoom-in font-black">
        <div className="flex gap-4 mb-10 p-2 bg-blue-50 rounded-2xl font-black">
          <button onClick={() => {setSubTab('dressing'); setIndex(0);}} className={`px-8 py-2 rounded-xl font-black text-xs uppercase ${subTab === 'dressing' ? 'bg-blue-500 text-white shadow-md' : 'text-slate-400'}`}>Dressing</button>
          <button onClick={() => {setSubTab('schedules'); setIndex(0);}} className={`px-8 py-2 rounded-xl font-black text-xs uppercase ${subTab === 'schedules' ? 'bg-blue-500 text-white shadow-md' : 'text-slate-400'}`}>Routine</button>
        </div>
        ...
        <div onClick={() => { onSound(subTab === 'dressing' ? `I need my ${(current as any).item} because ${(current as any).need}.` : `Let's follow our routine!`); onComplete(); }} className="w-full max-w-sm aspect-square bg-blue-50 rounded-[3rem] border-8 border-white shadow-inner flex items-center justify-center mb-10 overflow-hidden cursor-pointer group">
          {loading ? <Loader2 className="w-16 h-16 text-blue-400 animate-spin" /> : imageUrl && <img src={imageUrl} className="w-full h-full object-cover p-6" alt="practical life" />}
        </div>
        <h4 className="text-4xl font-black text-slate-800 uppercase mb-8">{(current as any).item || (current as any).name}</h4>
        <div className="flex gap-4">
          <button onClick={() => setIndex(i => (i === 0 ? data.length - 1 : i - 1))} className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center font-black"><ArrowLeft /></button>
          <button onClick={() => setIndex(i => (i + 1) % data.length)} className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center font-black"><ArrowRight /></button>
        </div>
      </div>
    );
};
...
const CommunicationModule: React.FC<{ onSound: (t: string) => void; onComplete: () => void }> = ({ onSound, onComplete }) => {
    ...
    const fetchVisual = useCallback(async () => {
        if (subTab !== 'pictureTalk' || !current || !schoolId) return;
        setLoading(true);
        const result = await generateLessonImageAction({ prompt: (current as any).prompt, schoolId });
        if (result.success) setImageUrl(result.data || null);
        setLoading(false);
      ...
      <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-orange-100 flex flex-col items-center animate-in zoom-in font-black">
        ...
        {subTab === 'pictureTalk' ? (
          <div onClick={handleAction} className="w-full max-w-2xl aspect-video bg-orange-50 rounded-[3rem] border-8 border-white shadow-inner flex items-center justify-center mb-10 overflow-hidden cursor-pointer group">
            {loading ? <Loader2 className="w-16 h-16 text-orange-400 animate-spin" /> : imageUrl && <img src={imageUrl} className="w-full h-full object-cover p-6" alt="talk" />}
          </div>
        ) : (
...
const SocialScenarios: React.FC<{ onSound: (t: string) => void; onComplete: () => void }> = ({ onSound, onComplete }) => {
    ...
    const generateWithAi = async () => {
      ...
      setIsAiLoading(true);
      try {
        const result = await generateSkillDetails({ skill: aiTopic, schoolId });
        if (result.success && result.data) {
            const details = result.data;
            if (subTab === 'community') {
                setCommunityData(prev => [...prev, { role: aiTopic, icon: 'fa-user-group', fact: details.description || `This is a helpful member of our community.`, prompt: details.imagePrompt || `A helpful person in a community setting, nursery style` }]);
            } else {
                setSocialData(prev => [...prev, { scenario: aiTopic, q: `What is the kindest thing to do?`, options: ['Help them', 'Play alone', 'Wait'], correct: 0, prompt: details.imagePrompt || `Children interacting kindly together` }]);
            }
    ...
    return (
      <div className="relative font-black">
        <button onClick={() => setIsDrawerOpen(true)} className="absolute -top-12 right-0 bg-white border-2 border-pink-200 text-pink-600 px-4 py-2 rounded-full font-black text-[10px] shadow-sm uppercase z-10 hover:bg-pink-50 transition-colors"><Wand2 className="w-3 h-3 mr-1 inline"/> AI Social Assistant</button>
        <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-pink-100 flex flex-col items-center min-h-[600px] animate-in slide-in-from-right duration-500">
          <h3 className="text-4xl font-black text-pink-500 mb-8 uppercase tracking-tighter text-center">Social & Emotional Hub 🤝</h3>
          
          <div className="flex gap-4 mb-10 p-2 bg-pink-50 rounded-2xl">
          ...
          {subTab === 'interaction' ? (
            <div className="flex flex-col items-center w-full">
              ...
              <div className="w-full max-w-lg aspect-video rounded-[3rem] border-8 border-white shadow-inner flex items-center justify-center mb-10 overflow-hidden bg-pink-50">
                {loading ? <Loader2 className="w-16 h-16 text-pink-400 animate-spin"/> : imageUrl && <img src={imageUrl} className="w-full h-full object-cover p-4 animate-in zoom-in" />}
              </div>
            ...
            <div className="flex flex-col items-center w-full animate-in zoom-in">
               ...
               <div onClick={() => onSound((current as any).fact)} className="relative w-full max-w-lg aspect-square bg-pink-50 rounded-[4rem] border-8 border-white shadow-2xl overflow-hidden mb-10 cursor-pointer group">
                 {loading ? <div className="absolute inset-0 flex items-center justify-center animate-spin"><Heart className="text-4xl text-pink-200"/></div> : imageUrl && <img src={imageUrl} className="w-full h-full object-cover p-6 transition-transform group-hover:scale-110" alt={(current as any).role} />}
              ...
              <div className="flex gap-4">
                <button onClick={() => setIndex(i => (i === 0 ? communityData.length - 1 : i - 1))} className="w-16 h-16 bg-slate-100 text-slate-600 rounded-full flex items-center justify-center shadow-md"><ArrowLeft/></button>
                <button onClick={() => { onSound((current as any).fact); onComplete(); }} className="px-12 py-5 bg-pink-500 text-white font-black rounded-3xl shadow-xl border-4 border-white uppercase tracking-widest">Learn Role! 🌟</button>
                <button onClick={() => setIndex(i => (i + 1) % communityData.length)} className="w-16 h-16 bg-slate-100 text-slate-600 rounded-full flex items-center justify-center shadow-md"><ArrowRight/></button>
              </div>
            </div>
          )}
        </div>
        {isDrawerOpen && <TeacherModal title={`Add ${subTab}`} topicLabel="Skill or Helper" topicValue={aiTopic} onTopicChange={setAiTopic} onGenerate={generateWithAi} isLoading={isAiLoading} onClose={() => setIsDrawerOpen(false)} />}
      </div>
    );
  };
...
const PuppetTheater: React.FC<{ onSound: (t: string) => void; onComplete: () => void }> = ({ onSound, onComplete }) => {
    ...
    const generateStory = async () => {
      if (!schoolId) return;
      setLoading(true);
      const rhymeResult = await generateRhyme({ topic: "Puppet Friends", schoolId });
      if (rhymeResult.success) {
          setStory(rhymeResult.rhyme);
          await onSound(`Welcome to the Puppet Theater! ${rhymeResult.rhyme}`);
          onComplete();
      }
      setLoading(false);
    };
    ...
    return (
      <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-emerald-100 flex flex-col items-center animate-in zoom-in font-black">
        ...
        <div className="flex gap-4 mt-12 font-black">
          <button onClick={() => setIndex(i => (i === 0 ? data.length - 1 : i - 1))} className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center"><ArrowLeft /></button>
          <button onClick={() => setIndex(i => (i + 1) % data.length)} className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center"><ArrowRight /></button>
        </div>
      </div>
    );
};
...
const PhysicalHealthModule: React.FC<{ onSound: (t: string) => void; onComplete: () => void }> = ({ onSound, onComplete }) => {
    ...
    const [grossMotor, setGrossMotor] = useState([
      { title: 'Froggy Jumps', action: 'Jump as high as you can like a green frog!', icon: 'fa-frog', prompt: 'A cute green frog jumping high in a meadow, nursery style' },
      { title: 'Super Runner', action: 'Run on the spot as fast as a superhero!', icon: 'fa-bolt', prompt: 'A cartoon child running very fast with motion lines, nursery style' },
    ]);
  
    const [fineMotor, setFineMotor] = useState([
      { title: 'String the Beads', action: 'Pretend to put a string through colorful beads!', icon: 'fa-circle-dot', prompt: 'Colorful large wooden beads on a thick string, nursery style' },
    ]);
  
    const [hygiene, setHygiene] = useState([
      { title: 'Wash Away Germs', action: 'Rub your hands together with bubbly soap!', icon: 'fa-soap', prompt: 'Happy hands covered in white soap bubbles, nursery style' },
    ]);
  
    const [nutrition, setNutrition] = useState([
      { title: 'The Green Giant', action: 'Eat your broccoli to be strong!', icon: 'fa-broccoli', prompt: 'A happy child eating a small green broccoli tree, nursery style' },
    ]);
    ...
    const generateWithAi = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
            ...
            const newItem = {
                title: aiTopic,
                action: details.description || `Let's practice our ${aiTopic} skill!`,
                icon: 'fa-star',
                prompt: details.imagePrompt || `A simple nursery drawing of ${aiTopic}`
            };
    ...
    return (
      <div className="relative font-black">
        <button onClick={() => setIsDrawerOpen(true)} className="absolute -top-12 right-0 bg-white border-2 border-green-200 text-green-600 px-4 py-2 rounded-full font-black text-[10px] shadow-sm uppercase z-10 hover:bg-green-50 transition-colors"><Wand2 className="w-3 h-3 mr-1 inline"/> AI Health Assistant</button>
...
const EmotionsModule: React.FC<{ onSound: (t: string) => void; onComplete: () => void }> = ({ onSound, onComplete }) => {
    ...
    return (
      <div className="relative font-black">
        <button onClick={() => setIsDrawerOpen(true)} className="absolute -top-12 right-0 bg-white border-2 border-yellow-200 text-yellow-600 px-4 py-2 rounded-full font-black text-[10px] shadow-sm uppercase z-10 hover:bg-yellow-50 transition-colors"><Wand2 className="w-3 h-3 mr-1 inline" /> AI Feeling Maker</button>
...
const LifeSkillsZone = ({ schoolId }: { schoolId: string }) => {
    ...
    const renderModule = () => {
      switch (activeTab) {
        case 'emotions': return <EmotionsModule onSound={playFeedbackSound} onComplete={addStar} />;
        case 'routine-songs': return <RoutineSongsModule onSound={playFeedbackSound} />;
        case 'modeling': return <ModelingModule onSound={playFeedbackSound} onComplete={addStar} />;
        case 'practical-life': return <PracticalLifeModule onSound={playFeedbackSound} onComplete={addStar} />;
        case 'communication': return <CommunicationModule onSound={playFeedbackSound} onComplete={addStar} />;
        case 'social': return <SocialScenarios onSound={playFeedbackSound} onComplete={addStar} />;
        case 'puppet-theater': return <PuppetTheater onSound={playFeedbackSound} onComplete={addStar} />;
        case 'cognitive': return <CognitiveSkills onSound={playFeedbackSound} onComplete={addStar} />;
        case 'physical-health': return <PhysicalHealthModule onSound={playFeedbackSound} onComplete={addStar} />;
