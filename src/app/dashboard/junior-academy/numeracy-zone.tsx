...
import {
    ...
    Sparkles, HelpCircle, Hash, ListOrdered, Scale, Type, Handshake, Plus, Minus,
    Layers, ObjectGroup, Clock, Coins, Ruler, Shapes, Move, CheckSquare, ArrowLeftRight,
    PenTool, BrainCircuit, Calculator, Apple, Star, Heart, Car, Zap, Cookie
} from 'lucide-react';
      ...
      <div className="relative font-black">
        ...
        <div className="w-full flex flex-col items-center bg-white p-10 rounded-[4rem] shadow-2xl border-8 border-purple-100 animate-in zoom-in min-h-[550px]">
          ...
          <div className="flex gap-6">
            <Button size="icon" onClick={() => setIndex(p => (p === 0 ? data.length - 1 : p - 1))} className="w-16 h-16 bg-slate-100 rounded-full"><ArrowLeft/></Button>
            <Button onClick={() => onSound(`Let's count to ${current.value}`)} className="px-16 h-16 bg-purple-500 text-white font-black rounded-2xl shadow-xl uppercase">Learn</Button>
            <Button size="icon" onClick={() => setIndex(p => (p + 1) % data.length)} className="w-16 h-16 bg-slate-100 rounded-full"><ArrowRight/></Button>
          </div>
        </div>
...
const NumberBondsModule: React.FC<{ onSound: (t: string) => void }> = ({ onSound }) => {
  ...
  return (
    <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-pink-100 flex flex-col items-center min-h-[550px]">
        ...
        {userAnswer === current.part2 && <Button onClick={() => setIndex((index + 1) % data.length)} className="mt-8 bg-pink-500 text-white rounded-2xl px-10 h-14">NEXT BOND</Button>}
    </div>
  );
};
  ...
  return (
    <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-yellow-100 flex flex-col items-center">
        ...
        <div className="w-full max-w-2xl aspect-video bg-yellow-50 rounded-[3rem] border-8 border-white mb-10 overflow-hidden">
          {imageUrl && <img src={imageUrl} className="w-full h-full object-cover p-6" />}
        </div>
        <div className="flex gap-6">
           {options.map(opt => (
  ...
  return (
    <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-emerald-100 flex flex-col items-center">
      ...
      <div className="flex gap-12">
        {current.items.map((item: any, idx: number) => (<Button key={idx} variant="ghost" onClick={() => { if(idx === current.correct) setAnswered(true); onSound(idx === current.correct ? "Yes" : "No"); }} className={cn("flex flex-col h-auto p-4 rounded-3xl border-4", answered && idx === current.correct ? 'border-green-400' : 'border-transparent')}><div className="w-48 h-48 bg-emerald-50 rounded-2xl mb-4 overflow-hidden">{imageUrls[idx] && <img src={imageUrls[idx]!} className="w-full h-full object-cover" />}</div><span className="text-xl font-black">{item.label}</span></Button>))}
      </div>
...
const SpatialModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => {
  ...
  return (
    <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-blue-100 flex flex-col items-center min-h-[550px]">
        ...
        <div className="w-full max-w-2xl aspect-video bg-blue-50 rounded-[3rem] overflow-hidden mb-10">
          {imageUrl && <img src={imageUrl} className="w-full h-full object-cover" />}
        </div>
