      ...
      <div className="relative font-black">
        ...
        <div className="w-full flex flex-col items-center bg-white p-10 rounded-[4rem] shadow-2xl border-8 border-purple-100 animate-in zoom-in min-h-[550px]">
          ...
          <div onClick={handleLearn} className="w-80 h-80 bg-purple-50 rounded-[3rem] border-8 border-white shadow-inner flex items-center justify-center mb-10 overflow-hidden cursor-pointer group">
            {loading ? <Loader2 className="w-16 h-16 animate-spin text-purple-400" /> : imageUrl && <img src={imageUrl} className="w-full h-full object-cover group-hover:scale-110 transition-all" alt="Number" />}
          </div>
        ...
        {isDrawerOpen && <TeacherModal title="AI Number Assistant" topicLabel="Enter a Number or Theme" topicValue={aiTopic} onTopicChange={setAiTopic} onGenerate={generateWithAi} isLoading={isAiLoading} onClose={() => setIsDrawerOpen(false)} />}
      </div>
    );
};
...
const NumberSequenceModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => {
    ...
    const generateWithAi = async () => {
        if (!aiTopic || !schoolId) return; setIsAiLoading(true);
        try {
            const result = await generateMathWorldEntry({ topic: aiTopic, category: 'sequence', schoolId });
            if(result.success && result.data) {
                // Expecting AI to return a JSON string in correctlyFormattedContent for sequences
...
const NumberComparisonModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => {
  ...
  const generateWithAi = async () => {
    if (!aiTopic || !schoolId) return; setIsAiLoading(true);
    try {
        const result = await generateMathWorldEntry({ topic: aiTopic, category: 'comparing', schoolId });
        if(result.success && result.data) {
            const parsed = JSON.parse(result.data.correctAnswer);
            setData(prev => [...prev, parsed]);
            setIndex(data.length); setIsDrawerOpen(false);
        ...
        <div className="w-full relative bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-orange-100 flex flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center gap-3">
                {Array.from({length: 21}).map((_, i) => (
                    <Button key={i} onClick={() => { setUserAnswer(i); onSound(i === correct ? "Perfect!" : "Keep counting"); }} className={cn("w-16 h-16 rounded-2xl font-black text-2xl shadow-md", userAnswer === i ? (i === correct ? 'bg-green-500 text-white' : 'bg-red-500 text-white') : 'bg-orange-50 text-slate-800')}>{i}</Button>
                ))}
            </div>
  ...
  return (
    <div className="w-full relative bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-indigo-100 flex flex-col items-center min-h-[550px]">
      ...
      <div className="flex items-center gap-12 mb-10 font-black">
         ...
         <div className="flex gap-4">
            <div className="text-center font-black"><p className="text-xs font-black text-indigo-500 uppercase tracking-widest mb-1 font-black">Tens</p><div className="w-16 h-16 border-4 border-indigo-200 text-indigo-600 rounded-xl flex items-center justify-center text-3xl font-black">{current.tens}</div></div>
            <div className="text-center font-black"><p className="text-xs font-black text-indigo-500 uppercase tracking-widest mb-1 font-black">Units</p><div className="w-16 h-16 border-4 border-indigo-200 text-indigo-600 rounded-xl flex items-center justify-center text-3xl font-black">{current.units}</div></div>
         </div>
      </div>
      <div className="w-full max-w-2xl aspect-video bg-indigo-50 rounded-[3rem] border-8 border-white overflow-hidden mb-10 cursor-pointer" onClick={() => onSound(`${current.number} has ${current.tens} ten and ${current.units} units`)}>
        {imageUrl && <img src={imageUrl} className="w-full h-full object-cover" alt="explanation"/>}
      </div>
