    ...
    return (
        ...
        <div className="w-full bg-white p-12 rounded-[4rem] shadow-2xl border-8 border-purple-100 flex flex-col items-center min-h-[550px]">
            ...
            {current.sequence.map((n, i) => (
                <div key={i} className={`w-24 h-32 rounded-3xl flex items-center justify-center border-4 text-5xl font-black ${n === null ? 'bg-purple-50 border-purple-100 text-purple-200 border-dashed' : (userAnswer === current.answer && n === null ? 'bg-green-500 text-white border-white' : 'bg-white border-purple-50 text-slate-800 shadow-md')}`}>
                {n === null ? (userAnswer !== null ? (userAnswer === current.answer ? userAnswer : '?') : '?') : n}
                </div>
            ))}
            </div>
            <div className="flex gap-4">
            {current.options.map(opt => (
                <button key={opt} onClick={() => handleAnswer(opt)} className={`w-20 h-20 rounded-2xl font-black text-3xl transition-all border-4 ${userAnswer === opt ? (opt === current.answer ? 'bg-green-500 text-white border-white' : 'bg-red-500 text-white border-white') : 'bg-purple-50 text-slate-700 border-purple-100 hover:bg-purple-100'}`}>{opt}</button>
            ))}
            </div>
        </div>
...
const NumberComparisonModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const NumberWordsModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const NumberBondsModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const AdditionModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const SubtractionModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const TensUnitsModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const GroupingModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const TellingTimeModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const MoneyCountingModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const MeasurementModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const ShapesModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const ComparisonGame: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const PatternGame: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const OneToOneGame: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;
const NumberMagicPen: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => <p>Coming Soon</p>;

/* --- SPATIAL REASONING (Position Words) --- */
const SpatialModule: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => {
    const [data, setData] = useState(constants.SPATIAL_DATA);
    const [index, setIndex] = useState(0);
    const [loading, setLoading] = useState(false);
    const [imageUrl, setImageUrl] = useState<string | null>(null);
    const [isDrawerOpen, setIsDrawerOpen] = useState(false);
    ...
    return (
      <div className="relative font-black">
        <button onClick={() => setIsDrawerOpen(true)} className="absolute -top-12 right-0 bg-white border-2 border-blue-200 text-blue-600 px-4 py-2 rounded-full font-black text-[10px] shadow-sm uppercase z-10 hover:bg-blue-50 transition-colors"><Wand2 className="h-3 w-3 mr-1 inline-block"/> AI Spatial Assistant</button>
