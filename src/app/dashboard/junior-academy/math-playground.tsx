...
const CountingGame: React.FC<{ onSound: (t: string) => void, schoolId: string }> = ({ onSound, schoolId }) => {
    ...
    const options = useMemo(() => {
        if (!current) return [];
        return [current.count, current.count + 1, current.count - 1].filter(o => o > 0).sort(() => Math.random() - 0.5)
    }, [current]);
    ...
    const handleAnswer = (val: number) => {
        ...
        if (val === current.count) {
            ...
            onSound(`Let's count them one by one!`);
        }
    };
    ...
    const generateWithAi = async () => {
        ...
        try {
            ...
            if(result.success && result.data){
                ...
                toast({ variant: 'destructive', title: 'AI Error', description: result.error || 'Failed to generate content.' });
            }
        } catch (e: any) { console.error(e); toast({ variant: 'destructive', title: 'Error', description: e.message }); }
