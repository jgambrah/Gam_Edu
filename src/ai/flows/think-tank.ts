...
const CrosswordPuzzleSchema = z.object({
  ...
  clues: z.object({
    ...
    down: z.array(z.object({
        ...
        row: z.number(),
        col: z.number(),
    })).optional().default([]), // Make down optional with a default value
  }),
});
...
export async function generateCrosswordAction(topic: string) {
  const prompt = `
    ...
    2. EVERY clue MUST include ALL 5 properties: number, clue, answer, row, col.
    3. 'row' and 'col' indicate where the word STARTS in the grid (0-indexed).
    4. Include both 'across' and 'down' arrays. If 'down' has no clues, you MUST return an empty array: "down": [].
    ...
    EXAMPLE OF CORRECT FORMAT:
    {
      ...
      "clues": {
        ...
        "down": [{"number": 2, "clue": "House pet", "answer": "CAT", "row": 0, "col": 0}]
      }
    }
    
    CREATE THE PUZZLE NOW with 4-6 words.
    `;
  ...
  try {
    ...
    output.clues.across = filterValidClues(output.clues.across);
    output.clues.down = filterValidClues(output.clues.down);
    
    if (output.clues.across.length === 0 && output.clues.down.length === 0) {
      throw new Error('No valid clues were generated by the AI.');
    }
