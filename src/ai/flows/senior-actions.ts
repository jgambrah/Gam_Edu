...
export async function generateSeniorEnglish(topic: string) {
  try {
    const prompt = `Generate an advanced literary passage for a senior student on the topic: "${topic}". Include a title, genre, difficulty, and exactly 3 critical thinking questions with answers.`;
...
export async function generateSeniorMath(concept: string) {
  try {
    const prompt = `Create an advanced math problem based on the concept: "${concept}". Provide a title, category, a complex LaTeX formula, an instruction, and a single, precise answer.`;
