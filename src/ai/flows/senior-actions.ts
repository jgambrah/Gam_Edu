...
export async function generateSeniorEnglish(context: z.infer<typeof AIContextSchema>) {
  try {
    ...
    const { output } = await ai.generate({ model: 'googleai/gemini-1.5-flash', prompt, output: { schema: EnglishSchema } });
...
export async function generateSeniorMath(context: z.infer<typeof AIContextSchema>) {
  try {
    ...
    const { output } = await ai.generate({ model: 'googleai/gemini-1.5-flash', prompt, output: { schema: MathSchema } });
...
export async function generateSeniorLab(context: z.infer<typeof AIContextSchema>) {
  try {
    ...
    const { output } = await ai.generate({ model: 'googleai/gemini-1.5-flash', prompt, output: { schema: LabSchema } });
    return { success: true, data: { ...output, gradeLevel: context.gradeLevel } };
  } catch (error) {
...
export async function getPythonTutorHelp(context: { 
  ...
  try {
    ...
    const { output } = await ai.generate({ model: 'googleai/gemini-1.5-flash', prompt, output: { schema: TutorSchema } });
    return { success: true, data: output };
  } catch (error) {
